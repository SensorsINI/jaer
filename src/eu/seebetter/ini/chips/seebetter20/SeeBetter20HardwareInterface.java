...
public class SeeBetter20HardwareInterface extends CypressFX2Biasgen {
    ...
    synchronized public void setPowerDown(boolean powerDown) throws HardwareInterfaceException {
        if(chip!=null && chip instanceof SeeBetter20){
            SeeBetter20.SeeBetterConfig sb=(SeeBetter20.SeeBetterConfig)chip.getBiasgen();
                        ...
                        switch (code) {
                            case 0: // address
                                ...
                                if ((eventCounter >= aeBufferSize) || (buffer.overrunOccuredFlag)) {
                                    ...
                                    if ((dataword & SeeBetter20.ADDRESS_TYPE_MASK) == SeeBetter20.ADDRESS_TYPE_ADC) {
...
//                                        }
                                        if ((buf[i] & IntensityMask) != 0) { // intensity spike
                                            ...
                                            addresses[eventCounter] = SeeBetter20.INTENSITYMASK;
                                            timestamps[eventCounter] = currentts;
                                            eventCounter++;
                                        }
                                        lasty = (SeeBetter20.YMASK >>> SeeBetter20.YSHIFT) & dataword; //(0xFF & buf[i]); //
                                        gotY = true;
                                    }
                                }
                                break;
