...
public class SBret10old extends ApsDvsChip {
	...
	public static final int MAX_ADC = (1 << 10) - 1;
	...
	public class SBret10Rendererold extends RetinaRenderer {
		...
		private double adc01normalized(double count) {
			double v;
			if (!agcEnabled) {
				v = (float) ((apsIntensityGain*count)+apsIntensityOffset) / MAX_ADC;
			} else {
		...
		private void putNextSampleValue(int val, ReadoutType type, int x, int y, int ts) {
			...
			switch (displayRead) {
				case A:
					...
					if(!(pushDisplay && (type==ReadoutType.ResetRead))) {
						pushDisplay=false;
					}
					break;
				case B:
					displayFrame[idx] = bBuffer[idx];
					if(!(pushDisplay && (type==ReadoutType.SignalRead))) {
						pushDisplay=false;
					}
					break;
					//                case C:
