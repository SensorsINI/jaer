...
class Davis208PixelParadeConfig extends DavisTowerBaseConfig {
	...
	public Davis208PixelParadeConfig(Chip chip) {
		...
		getPotArray().addPot(
				new TowerOnChip6BitVDAC(this, "Unconnected", 7, 7,
				"Unused, no effect"));
	...
	public class Davis208PixelParadeChipConfigChain extends DavisTowerBaseChipConfigChain {
		...
		OnchipConfigBit SelPreAmpAvgxD = new OnchipConfigBit(chip, "SelPreAmpAvgxD", 7, "If 1, connect PreAmpAvgxA to calibration neuron, if 0, commongate", true);
                OnchipConfigBit SelBiasRefxD = new OnchipConfigBit(chip, "SelBiasRefxD", 8, "If 1, select Nbias Blk1N, if 0, VDAC VblkV2", true);
                OnchipConfigBit nSelSensexD = new OnchipConfigBit(chip, "nSelSensexD", 9, "If 1, hook refractory bias to Vdd (unselect)", true);
                OnchipConfigBit nSelPosFbxD = new OnchipConfigBit(chip, "nSelPosFbxD", 10, "If 1, hook refractory bias to Vdd (unselect)", true);
		OnchipConfigBit nSelHpxD = new OnchipConfigBit(chip, "nSelHpxD", 11, "If 1, hook refractory bias to Vdd (unselect)", true);
		OnchipConfigBit SelGrayCounter = new OnchipConfigBit(chip, "SelGrayCounter", 13, "Selection of on-chip Gray counter for ADC", true);

		public Davis208PixelParadeChipConfigChain(Chip chip) {
			super(chip);
			//configBits[6].deleteObservers();
			//configBits[6] = null;
			configBits[7] = SelPreAmpAvgxD;
			configBits[7].addObserver(this);
			configBits[8] = SelBiasRefxD;
			configBits[8].addObserver(this);
			configBits[9] = nSelSensexD;
			configBits[9].addObserver(this);
                        configBits[10] = nSelPosFbxD;
			configBits[10].addObserver(this);
			configBits[11] = nSelHpxD;
			configBits[11].addObserver(this);
			configBits[13] = SelGrayCounter;
