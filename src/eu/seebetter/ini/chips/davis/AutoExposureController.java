...
public class AutoExposureController extends Observable implements HasPropertyTooltips {
	...
	public void controlExposure() {
		...
		if (pidControllerEnabled && (stats.maxNonZeroBin > 0)) {
			...
			final float err = (stats.meanBin - (stats.maxNonZeroBin / 2)) / (float) stats.maxNonZeroBin; // fraction of
																											// range
		...
		if ((stats.fracLow >= underOverFractionThreshold) && (stats.fracHigh < underOverFractionThreshold)) {
			...
			davisChip.getLog().log(Level.INFO, "Underexposed: {0} {1}", new Object[] { stats.toString(),
				String.format("expChange=%.2f (oldExposure=%10.3f newExposure=%10.3f)", expChange, currentExposure, newExposure) });
		}
	 ...
	 *            the underOverFractionThreshold to set
	 */
	...
	/**
	 * @param pidControllerEnabled
	 *            the pidControllerEnabled to set
	 */
	...
	/**
	 * @param centerWeighted
	 *            the centerWeighted to set
	 */
