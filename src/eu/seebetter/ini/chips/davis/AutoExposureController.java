...
public class AutoExposureController extends Observable implements HasPropertyTooltips {
    ...
    public AutoExposureController(final DavisBaseCamera davisChip) {
        ...
        expDelta = davisChip.getPrefs().getFloat("expDelta", 0.1F); // exposureControlRegister change if incorrectly exposed
        underOverFractionThreshold = davisChip.getPrefs().getFloat("underOverFractionThreshold", 0.2F); // threshold for fraction of total pixels that are underexposed
    ...
    public void setAutoExposureEnabled(final boolean yes) {
        final boolean old = this.autoExposureEnabled;
    ...
    public void controlExposure() {
        ...
        if ((davisChip.getAeViewer() != null) && (davisChip.getAeViewer().getPlayMode() != null) && (davisChip.getAeViewer().getPlayMode() != AEViewer.PlayMode.LIVE)) {
            return;
        }
        ...
        if ((stats.fracLow >= underOverFractionThreshold) && (stats.fracHigh < underOverFractionThreshold)) {
            ...
            davisChip.getLog().log(Level.INFO, "Underexposed: {0} {1}", new Object[]{stats.toString(), String.format("expChange=%.2f (oldExposure=%10.3f newExposure=%10.3f)", expChange, currentExposure, newExposure)});
            ...
            davisChip.getLog().log(Level.INFO, "Overexposed: {0} {1}", new Object[]{stats.toString(), String.format("expChange=%.2f (oldExposure=%10.3f newExposure=%10.3f)", expChange, currentExposure, newExposure)});
        } else {
