...
/**
 * IMU control of Invensense IMU-6100A, encapsulated here to control via
 * DavisConfig object that contains the low level control registers.
 */
public class ImuControl extends Observable implements HasPropertyTooltips, Biasgen.HasPreference, PreferenceChangeListener {
    ...
    public boolean isImuEnabled() {
        final boolean isAccelEnabled = ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunAccel")).isSet();
        final boolean isGyroEnabled = ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunGyro")).isSet();
        final boolean isTempEnabled = ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunTemp")).isSet();

        return (isAccelEnabled && isGyroEnabled && isTempEnabled);
    }

    public void setImuEnabled(final boolean yes) {
        ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunAccel")).set(yes);
        ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunGyro")).set(yes);
        ((SPIConfigBit) DavisConfig.getConfigValueByName(imuControl, "IMU.RunTemp")).set(yes);
    ...
    public final void loadPreference() {
        try {
            setImuEnabled(davisChip.getPrefs().getBoolean(getPreferencesKey() + "IMU.enabled", true));
    ...
    public void storePreference() {
        ...
        davisChip.getPrefs().putBoolean(getPreferencesKey() + "IMU.displayEnabled", isDisplayImu());
        davisChip.getPrefs().putBoolean(getPreferencesKey() + "IMU.enabled", isImuEnabled());
    }
...
//		}
        ...
        try {
            if (e.getKey().contains("IMU.AccelScale")) {
                ...
                setDisplayImu(Boolean.parseBoolean(e.getNewValue()));
            } if (e.getKey().contains("IMU.enabled")) {
                setImuEnabled(Boolean.parseBoolean(e.getNewValue()));
            }
