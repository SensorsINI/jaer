...
public class KalmanFilter extends EventFilter2D implements FrameAnnotater {//, PreferenceChangeListener {
    ...
    /**
     * setup the filter and geometry is recalculated
     */
    @Override
    ...
    /**
     * Here a packet of events is processed. First the method checks, if enough
     * events happened to calculate the Kalman Filter. If not, it happens
     * nothing, else for every cluster there is the prediction and update step
     * is done.
     *
    ...
    /**
     * The data class for each Cluster. In this data structure all the necessary
     * matrices for the Kalman Filter is stored. Each cluster is assigned to
     * such a structure. The class provides also the algorithm itself(prediction
     * and update step) of the KF.
     */
    final public class ClusterData {
        ...
        /**
         * The only constructor. For a Cluster it creates the necessary
         * datastructures.
         *
        ...
        /**
         * This method initializes the Kalman Filter for the supported cluster
         * depending on the actual cluster position and the chosen metric(see
         * mapToRoad parameter).
         */
        ...
        /**
         * The prediction step of the KalmanFilter. The new predicted state
         * vector xp and also the Error cov.Matrix P is calculated.
         */
        void predict() {
        ...
        /**
         * The update step for the Kalman Filter. As measurement the actual
         * position of the supported cluster is taken. Depending on the
         * metric(see map to road) the measurement is first translated in
         * meters.
         */
        void update() {
    ...
    /**
     * This method takes a point p. Depending on
     * parameters(distanceToVanishingPoint, distanceTo1Px, bridgeHeight) and
     * also depending on if a pixel is set as vanishing point, the distance in
     * meters is calculated. For the x value, the center of the window is
     * assumed to be 0 and the right side the positive one.
     *
    ...
    /**
     * This method calculates which y-coordinate a pixel has, that is a certain
     * distance ( in meters ) away.
     *
     ...
     *
     * @param meters The distance in meters
     * @return the y-coordinate of the pixel with that distance as a float(exact
     * position)
     */
    ...
    /**
     * This is a linear map that maps a cluster at the vanishing point to 0, and
     * at the buttom of the window the result would be 1.
     *
