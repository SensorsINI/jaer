 ...
 * <ul>
 * <li> "filterEnabled" - when the filter is enabled or disabled this event 
 *      is fired, if the subclass has not overridden the setFilterEnabled method
 * </ul>
...
public abstract class EventFilter extends Observable implements HasPropertyTooltips {
     ...
     * <pre> setPropertyTooltip(TOOLTIP_GROUP_GLOBAL, "propertyName", "property tip string"); </pre> */
     ...
     * a GUI for them is automatically built. Initially null - subclasses must make a new FilterChain and set it for this filter. */
    ...
    /** Every filter has a PropertyChangeSupport object. 
     * This support is used by the FilterPanel GUI to be informed of 
     * property changes when property setters fire a property change 
     * with the property name and old and new values.
     * This change in the property then will update the FilterPanel 
     * GUI control value. For example, the following shows how the 
     * boolean property <code>pathsEnabled</code> is handled.
     * <pre>
     ...
     *  }
     * </pre>
     * EventFilters can also use support for informing each other about 
     * changes in state.  An EventFilter declares itself to be 
     * a PropertyChangeListener and then adds itself to another 
     * EventFilters support as a PropertyChangeListener.
     * @return the support */
    ...
    /** Returns Description value of this filter as annotated by Description annotation. 
     * Note this method is not static and requires the EventFilter to 
     * already be constructed. The ClassChooser dialog uses the annotation 
     * to obtain class Descriptions without constructing the objects first.
     * @return the String description (the value() of the Description) or 
     * null if no description is available or any exception is thrown. */
    ...
    /** Developers can use setPropertyTooltip to add an optional tool-tip for 
     * a filter property so that the tip is shown as the tool-tip for the 
     * label or check-box property in the generated GUI.
     * <p>
