  ...
  //  protected float eventPoints[][] = new float[retinaSize][retinaSize];
            ...
            if(i>=0&&i<retinaSize){
                for (int j=y-radius; j<y+radius+1;j++){
                    if(j>=0&&j<retinaSize){
                            ...
                            if(limitRange){
                             // keep in range [0-1]
                                  if(filteredEventPoints[i][j]<0)filteredEventPoints[i][j]=0;
                                  else if(filteredEventPoints[i][j]>1)filteredEventPoints[i][j]=1;
                            }
    ...
    public void resetFilter() {
      ...
      //  eventPoints = new float[retinaSize][retinaSize];
        if(initGray)resetToGray(filteredEventPoints);
