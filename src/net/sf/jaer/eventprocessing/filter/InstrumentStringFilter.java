...
public class InstrumentStringFilter extends EventFilter2D implements Observer, FrameAnnotater {
    ...
    synchronized public EventPacket filterPacket(EventPacket in) {
        ...
        for (Object e : in) {
            if(e==null) break;  // this can occur if we are supplied packet that has data (e.g. APS samples) but no events @author tobi
    ...
    public void annotate(GLAutoDrawable drawable) {
        ...
        if(!isFilterEnabled()) return; // @author Adrien: what?
        GL2 gl=drawable.getGL().getGL2();   
        GL2 colorCodeGL = drawable.getGL().getGL2();
        GL2 lastEventsAvgGL = drawable.getGL().getGL2();
        
        if (gl == null) return;
        if (colorCodeGL == null) return;
        if (lastEventsAvgGL == null) return;
