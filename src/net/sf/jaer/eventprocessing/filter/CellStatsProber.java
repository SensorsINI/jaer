...
public class CellStatsProber extends EventFilter2DMouseROI implements FrameAnnotater, MouseListener, MouseMotionListener, Observer,
    ...
    private float histogramLineWidth = getFloat("histogramLineWidth", 2f);
    ...
    public CellStatsProber(AEChip chip) {
        ...
        setPropertyTooltip(h, "histogramLineWidth", "Line thickness of histograms");
                        ...
                        if (showIndividualISIHistograms) {
                            ...
                            for (IsiOrFreqHist h : histMap.values()) {
                                ...
                                if (!(mouseHistAddress == h.address)) {
                                    ...
                                    h.draw(gl, getHistogramLineWidth()*2, SELECT_COLOR);
//                                    renderer.begin3DRendering();
...
//                                    renderer.end3DRendering();
                        ...
                        if (showAverageISIHistogram) {
                            ...
                            globalHist.draw(gl, getHistogramLineWidth(), GLOBAL_HIST_COLOR);
                            gl.glPopMatrix();
                        }
                        if (showRateDistribution) {
                            gl.glPushMatrix();
                            RATE_HIST_COLOR[3] = histogramOpacity;
                            rateHist.draw(gl, getHistogramLineWidth(), RATE_HIST_COLOR);
                            gl.glPopMatrix();
                        }
    ...
    /**
     * @return the histogramLineWidth
     */
    public float getHistogramLineWidth() {
        return histogramLineWidth;
    }

    /**
     * @param histogramLineWidth the histogramLineWidth to set
     */
    public void setHistogramLineWidth(float histogramLineWidth) {
        this.histogramLineWidth = histogramLineWidth;
        putFloat("histogramLineWidth",histogramLineWidth);
    }
