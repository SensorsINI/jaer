...
/**
 * An AE input stream that uses a generic InputStream such as a URL connection
 * to an http server or memory input stream.
 *
 * @author tobi
 */
public class AEInputStream implements Closeable {
    ...
    /**
     * Property change event. Listeners should check for events using these
     * static String objects.
     * <p>
     * In order to use these events, an EventFilter must register itself either
     * with the AEInputStream or with the owning AEViewer. But this registration
     ...
     * <li>EVENT_POSITION means input stream position has changed (called just
     * before returning each new packet)
     ...
     * <li>EVENT_MARKS_LOADED means new marks loaded, new value are the marks
     * for this file
     * <li>EVENT_INIT is called after initialization of the input stream, with
     * new value this
     * <li>EVENT_NON_MONOTONIC_TIMESTAMP a non-monotonic timestamp was detected
     * <li>EVENT_REPOSITIONED the input stream has been repositioned (e.g. by
     * using slider in AEPlayer) to a new position
     * </ul>
     */
    public static final String EVENT_EOF = "eof",
            ...
            EVENT_MARKS_LOADED = "marksLoaded",
            EVENT_MARK_IN_SET = "markInSet",
            EVENT_MARK_OUT_SET = "markOutSet",
            EVENT_MARK_TOGGLED = "markToggled",
    ...
    /**
     * Constructs new instance of AEInputStream, and attempts skip its header,
     * if any.
     *
    ...
    /**
     * @param addressType the addressType to set, either Integer.TYPE or
     * Short.TYPE
     */
