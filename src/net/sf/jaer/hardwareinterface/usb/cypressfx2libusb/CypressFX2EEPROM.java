...
/**
 * Utility GUI for dealing with CypressFX2 EEPROM stuff. Using this rudimentary tool, you can scan for USBIO devices. If
 * the device is virgin (not had
 * its EEPROM programmed) then it will be found and the retina firmware will be downloaded to it. This firmware (at
 * least on the retina board) allows for
 * programming the Cypress's EEPROM. You can then program a desired C0 (VID/PID) or C2 (firmware) load into the device's
 * EEPROM.
 * <p>
 * You can also program firmware into the device RAM directly (using the built-in Cypress hardare vendor request to
 * write to device RAM). Firmware files can be downloaded to the device's RAM from two source formats, a flat binary
...
public class CypressFX2EEPROM extends javax.swing.JFrame {
	...
	AEChip chip;
	short VID = (short) 0x0547, PID = (short) 0x8700, DID = 0; // defaults to tmpdiff128 retina, TO-DO fix this lousy
	// default
	...
	public CypressFX2EEPROM() {
		...
		final boolean b = prefs.getBoolean("CypressFX2EEPROM.writeEEPROMRadioButton", true);
		if (b) {
	...
	synchronized private void scanForUsbIoDevices() {
		try {
			cypress = (CypressFX2) LibUsbHardwareInterfaceFactory.instance().getFirstAvailableInterface();
			if (cypress == null) {
			...
			final String devString = cypress.toString();
			...
			if (cypress.getNumberOfStringDescriptors() > 2) {
				writeDeviceIDTextField.setText(cypress.getStringDescriptors()[2]);
	...
	void enableDeviceIDProgramming(final boolean yes) {
	...
	void setButtonsEnabled(final boolean yes) {
	...
	private void initComponents() {
		...
		final org.jdesktop.layout.GroupLayout USBAERmini2panelLayout = new org.jdesktop.layout.GroupLayout(
			USBAERmini2panel);
		...
		USBAERmini2panelLayout.setHorizontalGroup(USBAERmini2panelLayout.createParallelGroup(
			...
			USBAERmini2panelLayout
				...
				.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
				.add(jPanel4, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 452,
					org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addContainerGap(118, Short.MAX_VALUE)));
		USBAERmini2panelLayout.setVerticalGroup(USBAERmini2panelLayout
			.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(
		...
		final org.jdesktop.layout.GroupLayout CPLDDownloadPanelLayout = new org.jdesktop.layout.GroupLayout(
			CPLDDownloadPanel);
	...
	private void downloadCPLDFirmwareButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_downloadCPLDFirmwareButtonActionPerformed
		if (!checkDeviceReallyOpen()) {
			return;
		}
		final File f = new File(CPLDfilenameField.getText());

		if (!f.exists()) {
			JOptionPane.showMessageDialog(this,
				"File doesn't exist. Please choose a binary download CPLD image file (.xsvf) first.");
			return;
		}

		final Thread T = new Thread("CPLD download") {

			@Override
			public void run() {
				try {
					setWaitCursor(true);
					// ProgressMonitor progressMonitor=new ProgressMonitor(chip.getAeViewer(),
					// "Downloading firmware to EEPROM","", 0, task.getLengthOfTask());
	...
	private void chooseCPLDFileButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_chooseCPLDFileButtonActionPerformed
		final JFileChooser chooser = new JFileChooser(prefs.get("CypressFX2EEPROM_CPLD.filepath", ""));
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				CPLDfilenameField.setText(file.getCanonicalPath());
				CPLDfilenameField.setToolTipText(file.getCanonicalPath());
	...
	private void writeRAMRadioButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_writeRAMRadioButtonActionPerformed
		prefs.putBoolean("CypressFX2EEPROM.writeEEPROMRadioButton", false);
	}// GEN-LAST:event_writeRAMRadioButtonActionPerformed

	private void writeEEPROMRadioButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_writeEEPROMRadioButtonActionPerformed
	...
	private void chooseFileButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_chooseFileButtonActionPerformed
		...
		final FileFilter filter = new FileFilter() {
			...
			@Override
			public boolean accept(final File f) {
				// if(f.getName().toLowerCase().endsWith(".iic") || f.getName().toLowerCase().endsWith(".hex") ||
				// f.isDirectory())
	...
	void setEEPROMRAMRadioButtonsAccordingToFileType(final File f) {
	...
	private void writeDeviceIDButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_writeDeviceIDButtonActionPerformed
		if (!checkDeviceReallyOpen()) {
			return;
		}
		try {
	...
	private void eraseButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_eraseButtonActionPerformed
		final int retVal = JOptionPane.showConfirmDialog(this, "Are you sure you want to erase the EEPROM?",
			"Really erase?", JOptionPane.YES_NO_OPTION);
		...
		final Thread T = new Thread("Firmware eraser") {
			...
			public void run() {
				try {
					...
					JOptionPane.showMessageDialog(CypressFX2EEPROM.this,
						"EEPROM erase successful - unplug and replug the device to complete erase",
	...
	void setWaitCursor(final boolean yes) {
		if (yes) {
	...
	private void downloadFirmwareButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_downloadFirmwareButtonActionPerformed
		...
		final boolean isHexFile = f.getName().toLowerCase().endsWith(".hex");
		final boolean isBixFile = f.getName().toLowerCase().endsWith(".bix");
		final boolean isIicFile = f.getName().toLowerCase().endsWith(".iic");

		final boolean fIsBixFile = isBixFile, fIsHexFile = isHexFile, fIsIICFile = isIicFile;
		final Thread T = new Thread("FX2 firmware download") {

			@Override
			public void run() {
				try {
					setWaitCursor(true);
					final boolean toRam = writeRAMRadioButton.isSelected();
					if (fIsBixFile) {
						if (toRam) {
							try {
								cypress.open();
							}
							catch (final BlankDeviceException e) {
								log.info(e.getMessage());
							}
							cypress.downloadFirmwareBinary(filenameTextField.getText());
							// cypress.resetUSB(); // don't need this which in any case would fail because device is not
							// open or configured
							// cypress.cyclePort();
						}
						else {
							JOptionPane.showMessageDialog(CypressFX2EEPROM.this,
								"bix files cannot be written to EEPROM, only to RAM.");
						}
					}
					else if (fIsIICFile) {
						if (toRam) {
							JOptionPane.showMessageDialog(CypressFX2EEPROM.this,
								"iic files cannot be written to RAM, only to EEPROM.");
						}
						else {
							if (!checkDeviceReallyOpen()) {
								return;
	...
	private void scanButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_scanButtonActionPerformed
		scanForUsbIoDevices();
	}// GEN-LAST:event_scanButtonActionPerformed

	private void writeVIDPIDDIDButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_writeVIDPIDDIDButtonActionPerformed
		if (!checkDeviceReallyOpen()) {
			return;
		}
		...
		log.info("Writing VID/PID/DID " + HexString.toString(VID) + "/" + HexString.toString(PID) + "/"
	...
	public boolean isExitOnCloseEnabled() {
		return exitOnCloseEnabled;
	}

	/**
	 * @param exitOnCloseEnabled
	 *            set true so that exit really exits JVM. default false only disposes window
	 */
