...
public class CypressFX2TmpdiffRetinaHardwareInterface extends CypressFX2Biasgen implements HasResettablePixelArray {
    ...
    public class RetinaAEReader extends CypressFX2.AEReader {
        ...
        @Override
        /**
         * Populates the AE array, translating from raw buffer data to raw
         * addresses and unwrapped timestamps.
         * <p>
         * Event addresses and timestamps are sent from USB device in BIG-ENDIAN
         * format. MSB comes first, The addresses are simply copied over, and
         * the timestamps are unwrapped to make 32 bit int timestamps.
         * <p>
         * Wrapping is detected differently depending on whether the hardware
         * sends empty wrap packets or simply wraps the timestamps. When the
         * timestamps simply wrap, then a wrap is detected when the present
         * timestamp is less than the previous one. Then we assume the counter
         * has wrapped--but only once--and add into this and subsequent
         * timestamps the wrap value of 2^16. this offset is maintained and
         * increases every time there is a wrap. Hence it integrates the wrap
         * offset.
         *
         * <p>
         * Newer USB monitor interfaces based on using a CPLD pumping data into
         * the CypressFX2 in slave FIFO mode signal a timestamp wrap by sending
         * an empty wrap event;see {@link #translateEvents_EmptyWrapEvent}.
         *
         * <p>
         * The timestamp is returned in 1 us ticks. This conversion is to make
         * values more compatible with other CAVIAR software components.
         * <p>
         * If an overrun has occurred, then the data is still translated up to
         * the overrun.
         *
     ...
     *
     * @param value true to reset the pixels (hold them from spiking), false to
