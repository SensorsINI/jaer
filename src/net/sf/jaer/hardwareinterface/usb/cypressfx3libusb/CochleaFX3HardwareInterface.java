                    ...
                    if ((event & 0x8000) != 0) {
                        ...
                        switch (code) {
                            ...
                            case 7: // Timestamp wrap
                                ...
                                // TODO generate timer event to advance clock on host side even with low event rate of cochlea
                                CypressFX3.log.fine(
                                        String.format("Timestamp wrap event received on %s with multiplier of %d.", super.toString(), data));

                                if (ensureCapacity(buffer, eventCounter + 1)) {
                                    buffer.getAddresses()[eventCounter] = ((int)(data&0xFFFF)) | BasicEvent.SPECIAL_EVENT_BIT_MASK;
