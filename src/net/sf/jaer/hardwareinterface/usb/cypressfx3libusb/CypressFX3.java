...
public class CypressFX3 implements AEMonitorInterface, ReaderBufferControl, USBInterface, HasUsbStatistics {
    ...
    public PacketBundle acquireAvailablePacketBundle() throws HardwareInterfaceException {
        ...
        synchronized (packetBundlePool) {
            ...
            beforeAuthoritativeTypedBundleSwap(writeBundle);
    ...
    /**
     * Lets subclasses append side-channel packets while the typed write buffer
     * is locked and before its metadata is sealed.
     */
    ...
    /**
     * Check the firmware and logic versions against exact required ones and
     * display an error if not met.
     */
...
//			throw new HardwareInterfaceException(updateString);
     ...
     * time {@link #acquireAvailableEventsFromDriver } was called. This flag is
    ...
    public void close() {
        ...
        synchronized (lifecycleLock) {
            ...
            readerToStop = aeReader;
        }

        log.info("Closing " + getClass().getSimpleName());
        boolean readerDead = true;
        boolean closeCapturedHandle = false;
        try {
            ...
            try {
                if (readerToStop != null) {
                    ...
                    readerDead = stopAEReader(readerToStop);
                }
            ...
            if (readerDead) {
                ...
                // The reader may still own native transfers. Record logical closure,
                // but make no further USB calls on this handle.
                inEndpointEnabled = false;
            }
            ...
            if (handleToClose != null) {
                // Fail closed if the abandonment decision itself fails. Native
                // teardown is safe only after terminal reader proof.
                boolean abandoned = true;
                try {
                    abandoned = UsbAsyncBulkReaderLifecycle.abandonNativeHandle(readerDead, log, "CypressFX3");
                } catch (final RuntimeException e) {
                    log.warning(String.format("Error checking native handle ownership: %s", e.toString()));
                }

                if (readerDead && !abandoned) {
                    // Set before reset/release so either failure still reaches the
                    // single safe LibUsb.close in finally.
                    closeCapturedHandle = true;
                }

                if (closeCapturedHandle && shouldResetUsbDevice()) {
                    try {
            ...
            // but must be abandoned if a transfer thread may still be inside USB.
            if (handleToClose != null && closeCapturedHandle) {
                try {
    ...
    private boolean stopAEReader(final AEReader reader) {
        if (reader != null) {
            ...
            try {
                stopped = reader.stopThread();
            } catch (final RuntimeException e) {
                stopFailure = e;
            }

            final boolean terminal = stopFailure == null && stopped && reader.isTerminal();
            synchronized (lifecycleLock) {
                if (terminal) {
                    if (aeReader == reader) {
                        aeReader = null;
...
//			configSequence.sendConfigSequence();
    ...
    public class AEReader implements ReaderBufferControl {
         ...
         * CypressFX3 (512 bytes). If it is too small, then there are frequent
        ...
        private boolean authoritativeTypedCallback;
        ...
        private void startBulkTransferThread(long generation) throws HardwareInterfaceException {
            ...
            usbTransfer.start();
            final long handshakeMs = monitor.queueHandshakeMs();
            if (handshakeMs > 0 && !UsbTransferSubmit.awaitQueued(
                    usbTransfer, handshakeMs, CypressFX3.log, "CypressFX3 AEReader")) {
                readerActive = false;
        ...
        protected final boolean isAuthoritativeTypedCallback() {
            return authoritativeTypedCallback;
        }
        ...
        public boolean stopThread() {
            ...
            if (!stopped) {
                bufferLifecycle.markFailed();
                // Do not start CypressFX3-USB-recover while close owns this
                // generation; that path could close a live native reader.
                if (!closeInProgress && !Thread.holdsLock(monitor)) {
        ...
        class ProcessAEData implements RestrictedTransferCallback {
            ...
            public void processTransfer(final RestrictedTransfer transfer) {
                ...
                final boolean authoritativeTyped = monitor.isAuthoritativeTypedDelivery();
                if (authoritativeTyped) {
                    synchronized (packetBundlePool) {
                        processTransferLocked(transfer, authoritativeTyped);
                    }
                } else {
                    synchronized (aePacketRawPool) {
                        processTransferLocked(transfer, authoritativeTyped);
            ...
            private void processTransferLocked(final RestrictedTransfer transfer,
                    final boolean authoritativeTyped) {
                if (!bufferLifecycle.isCurrent(generation)) {
                    return;
                }
                authoritativeTypedCallback = authoritativeTyped;
                try {
                    ...
                    authoritativeTypedCallback = false;
                }
            }
        }
    ...
    /** Queue-start handshake is opt-in so existing DAVIS/SciDVS starts do not wait. */
    ...
    public synchronized void setEventAcquisitionEnabled(final boolean enable) throws HardwareInterfaceException {
        ...
        if (enable) {
            try {
                startAEReader();
            } catch (final HardwareInterfaceException | RuntimeException startFailure) {
                try {
                    setInEndpointEnabled(false);
                } catch (final HardwareInterfaceException rollbackFailure) {
...
//        if (false) { // TODO: does this really ever happen, a
     ...
     * @param index the "index" of the request (bIndex USB field)
     * @param bytes the data which is to be transmitted to the device
     * @param pos position at which to start consuming the bytes array
     * @param length number of bytes to copy, starting at position pos
     */
     ...
     * blocking method.
     *
     * @param requestType the vendor requestType byte (used for special cases,
     * usually 0)
     * @param request the vendor request byte, identifies the request on the
     * device
     * @param value the value of the request (bValue USB field)
     * @param index the "index" of the request (bIndex USB field)
     * @param dataBuffer the data which is to be transmitted to the device (null
     * means no data)
     */
     ...
     * @param value the value of the request (bValue USB field)
     * @param index the "index" of the request (bIndex USB field)
     * @param dataLength amount of data to receive, determines size of returned
    ...
    void recoverFailedBufferReconfig(Exception cause) {
        ...
        synchronized (lifecycleLock) {
            if (closeInProgress) {
                CypressFX3.log.warning("CypressFX3 close already owns this generation; skipping USB-recover");
                return;
            }
        }
        ...
        new Thread(() -> {
            try {
                ...
                CypressFX3.log.warning("CypressFX3 recover close failed: " + e);
            }
        }, "CypressFX3-USB-recover").start();
    }
    ...
    public void setAeReader(final AEReader aeReader) {
        synchronized (lifecycleLock) {
            ...
            } else if (this.aeReader != null) {
                if (closeInProgress) {
                    throw new IllegalStateException("Cannot clear AEReader while device close is in progress");
                }
                if (this.aeReader == nonterminalReaderGeneration && hasNonterminalReaderGenerationLocked()) {
                    throw new IllegalStateException("Cannot clear a nonterminal AEReader generation");
                }
            }
