...
public class USBPacketStatistics implements HasUsbStatistics {
    ...
    public Histogram packetSizeHistogram=new SimpleHistogram(0,128,30,0), packetIntervalUsHistogram=new SimpleHistogram(0, 125, 30, 0);
    int nSamples=0;
    ...
    private void addValues(int nBytes) {
        if(!(showEnabled || printEnabled)) return;
        if(!initialized){
        ...
        packetIntervalUsHistogram.add(dtUs);
        packetSizeHistogram.add(nBytes);
        if(printEnabled) printStatistics();
        if(showEnabled) showStatistics();
    }
   ...
   public void printStatistics(){
       if(nSamples%INTERVAL!=0) return;
       System.out.println("Packet size (bytes) histogram:");
       packetSizeHistogram.print();
       System.out.println("\nPacket interval (us) histogram:");
