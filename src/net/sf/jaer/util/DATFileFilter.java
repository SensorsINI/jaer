...
public class DATFileFilter extends javax.swing.filechooser.FileFilter {
    ...
    public enum Category {
        ...
        ALL_RECOGNIZED,
        /** Recognized event-data extensions shown in the hardware-sequencing chooser. */
        SEQUENCING_DATA,
        /** {@code .aedat} only. */
        AEDAT,
        /** {@code .aedat2} only. */
    ...
    /**
     * Installs the hardware-sequencing chooser filters. Index playlists are not
     * offered because sequencing reads one event-data file directly through the
     * legacy {@code AEFileInputStream} path rather than playlist dispatch.
     *
     * @param chooser the sequencing chooser to configure
     */
    public static void installSequencingDialogFilters(JFileChooser chooser) {
        chooser.resetChoosableFileFilters();
        chooser.setAcceptAllFileFilterUsed(false);
        DATFileFilter sequencingData = new DATFileFilter(Category.SEQUENCING_DATA);
        chooser.addChoosableFileFilter(sequencingData);
        chooser.addChoosableFileFilter(new DATFileFilter(Category.ALL_FILES));
        chooser.setFileFilter(sequencingData);
    }
    ...
    public boolean accept(File f) {
        ...
        switch (category) {
            ...
            case SEQUENCING_DATA:
                return isSequencingDataExtension(extension);
            case OTHER:
    ...
    /** True for recognized event-data files, excluding index playlists. */
    public static boolean isSequencingDataExtension(String extension) {
        return isRecognizedExtension(extension)
                && !INDEX_EXTENSION.equals(extension)
                && !OLD_INDEX_EXTENSION.equals(extension);
    }
    ...
    public String getDescription() {
        switch (category) {
            ...
            case SEQUENCING_DATA:
                return "Sequencing event data (*.aedat4, *.aedat2, *.aedat, *.aedz, *.dat, *.raw, *.h5, *.hdf5, *.bag, *.csv, *.txt)";
            case OTHER:
                return "Other event data (*.aedz, *.dat, *.raw, *.h5, *.hdf5, *.bag, *.csv, *.txt, *.aeidx, *.index)";
            case ALL_FILES:
                return "All files and folders";
            case ALL_RECOGNIZED:
            default:
                return "All recognized event data (*.aedat4, *.aedat2, *.aedat, *.aedz, *.dat, *.raw, *.h5, *.hdf5, *.bag, *.csv, *.txt, *.aeidx, *.index)";
        }
    }
