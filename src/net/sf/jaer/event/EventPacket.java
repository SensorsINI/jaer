 ...
 * * Note that events that are "filteredOut" by a preceding filter (by it calling the setFilteredOut method on the
 * event) may not be filtered out
 * by the InputIterator of this packet if they are the last event in a packet. Individual filters can still check the
 * BasicEvent's isFilteredOut() method.</b>
 *
 *
 *
 * @param <E>
 *            the class of the events in this packet
 * @author tobi
 *
 * @see BasicEvent
...
public class EventPacket<E extends BasicEvent> implements /* EventPacketInterface<E>, */ Cloneable, Iterable<E> {
	...
	static public void restartTimeLimiter() {
		EventPacket.timeLimitTimer.restart();
	}
	 ...
	 *            time in ms
	 */
	public static void restartTimeLimiter(final int timeLimitMs) {
		EventPacket.setTimeLimitMs(timeLimitMs);
		EventPacket.restartTimeLimiter();
	}
	...
	/**
	 * Returns the raw data packet that this originally came from. These are the raw ints that represent the data from
	 * the device.
	 ...
	 * the InItr hasNext() iteration over the packet. Therefore it is only valid after the iteration over the packet is
	 * finished.
	 * <p>
	 * This count does <b>not</b> include the events set to be filteredOut during the previous iteration, only the ones
	 * that were already filtered out
	...
	public class InItr implements Iterator<E> {
		...
		public void reset() {
			cursor = 0;
			usingTimeout = EventPacket.timeLimitTimer.isEnabled(); // timelimiter only used if timeLimitTimer is enabled
																	// but flag to
	...
	/**
	 * Ensures packet has room for n events. The original events are retained and the new capacity is filled with
	 * default
	 * events.
	 *
	 * @param n
	 ...
	 * If the packet has extra data not consisting of events this method could return 0 but there could still be data,
	 * e.g. sampled ADC data, image frames, etc.
	 *
	 *
	...
	/**
	 * Reports if the packet is empty. The default implementation reports true if size in events is zero, but subclasses
	 ...
	 *
	 * @param constructor
	 *            - a zero argument constructor for the new events.
	 */
	...
	final public static int getTimeLimitMs() {
		return EventPacket.timeLimitTimer.getTimeLimitMs();
	}
	 ...
	 */
	final public static void setTimeLimitMs(final int timeLimitMs) {
		EventPacket.timeLimitTimer.setTimeLimitMs(timeLimitMs);
	}

	final public static void setTimeLimitEnabled(final boolean yes) {
		EventPacket.timeLimitTimer.setEnabled(yes);
	}
	 ...
	 *
	 * @param elementData
	 *            the underlying element data, which should extend BasicEvent
	 */
