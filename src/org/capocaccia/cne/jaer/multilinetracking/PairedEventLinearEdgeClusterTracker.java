/*
 * PairedEventLinearEdgeClusterTracker.java
 *
 ...
 * pairs of recent events. The event pairs come from a buffer formed from recent
 * events. Each pair defines a line with polar and angle parameters. Lines are
 * tracked using polar (rho) and angle (theta) parameters in a space of
 * rho/theta, in analogy with the PairedEventLinearEdgeClusterTracker tracking
...
public class PairedEventLinearEdgeClusterTracker extends EventFilter2D implements FrameAnnotater, Observer, LineDetector {
	...
	synchronized private void track(EventPacket<BasicEvent> packet) {
		...
		checkOutputPacketEventType(packet); // for apsDVS event bypass
	...
	/**
	 * Computes signed distance to-from between two angles with cut at -PI,PI.
	 * E.g. if e is from small angle and from=PI-e, to=-PI+e, then angular
	 * distance to-from is -2e rather than (PI-e)-(-PI+e)=2PI-2e. This minimum
	 * angle difference is useful to push an angle in the correct direction by
	 * the correct amount. For this example, we want to push an angle hovering
	 * around PI-e. We don't want angles of -PI+e to push the angle from lot,
	 * just from bit towards PI. If we have angle
	 * <code>from</code> and new angle
	 * <code>to</code> and mixing factor m<<1, then new *
	 * angle <code>c=from+m*angleDistance(from,to)</code>.
	 *
	 *
	...
	/**
	 * A LineSegment is formed from a pair of events and has a rho and theta
	 * that characterize it, along with other parameters. Note that rho theta
	 * originate in LL chip image corner.
	 */
	private class LineSegment {
		//        BasicEvent a=null, b=null; // we don't use these because they can getString referenced out from under us

		/**
		 * theta is angle in radians from 0 to PI with 0 and PI horizontal
		 * normals to line segment
		 */
		double thetaRad = 0;
		/**
		 * rhoPixels is distance of line segment closest passage to origin of
		 * chip image LL corner
		 */
		double rhoPixels = 0;
		int dx = 0, dy = 0, dt = 0;
		...
		/**
		 * computes the rho (pixels distance from origin at LL corner) and theta
		 * (radian angle CCW from 0 horizontal) for the segment given the two
		 * events. This is confusing because a raw calculation can give negative
		 * rho and angle that spans -PI to PI. This values are unambiguous about
		 * specifying the line segment but similar segments can have very
		 * different rho/theta values depending on the quadrant that dx,dy are
		 * in. Also, the rho values -PI and PI are identical although they
		 * differ by 2*PI. This makes combining values confusing. Therefore the
		 * values are rationalized by ensuring positive rho and forcing theta
		 * between -PI to PI.
		 */
	...
	/**
	 * Checks two events to see if they could form a valid line segment. The
	 * events need to have the same Polarity is the event is a PolarityEvent.
	 *
	 * @return segment if line segment is long enough but not too long and is
	 * not based on events too far apart in time. Returns null if not a goog
	 * segment
	 */
	...
	/**
	 * Method that given segment event, returns closest cluster and distance to
	 * it. The actual computation returns the first cluster that is within the
	...
	/**
	 * Represents a single tracked line. It is characterized mainly by the
	 * normal to it from the chip image LL corner. rhoPixels is the length of
	 * the normal in pixels and thetaRad is the angle of the normal in radians
	 * CCW from x-axis.
	 */
	public class LinearFeatureModel {
		...
		/**
		 * Angle of normal to line CCW from x axis with 0 and PI being
		 * horizontal and PI/2 being vertical. thetaRad spans -PI to PI.
		 */
		private double thetaRad = 0;
		/**
		 * velocity of cluster in pixels/tick, where tick is timestamp tick
		 ...
		 * weighted by the number of events that have supported each source
		...
		public LinearFeatureModel(LinearFeatureModel one, LinearFeatureModel two) {
			...
			//            LinearFeatureModel older=one.numEvents>two.numEvents? one:two;
		...
		/**
		 * computes metric of distance in rho/theta space, normalizing each by
		 * appropriate size: rho by chip.getMaxSize() and theta by Math.PI.
		 * Returns normalized distance metric which can be used for searching
		 * for closest LinearFeatureModel to a LineSegment
		 *
		...
		/**
		 * @return distance in pixels between line segment and line cluster rho
		 * metric
		 */
		...
		/**
		 * @return distance of this cluster to the other cluster, abs value in
		 * normalized units combining rho and theta
		 */
		 ...
		 *
		 * @return true if LinearFeatureModel contains segment, i.e., rho is
		 * within rhoRadius and thetaRad is within thetaRadiusRad
		 */
		...
		/**
		 * @return averaged velocity of cluster in pixels per second. The
		 * velocity is instantaneously computed from the movement of the cluster
		 * caused by the last event, then this velocity is mixed with the the
		 * old velocity by the mixing factor. Thus the mixing factor is appplied
		 * twice: once for moving the cluster and again for changing the
		 * velocity.
		 */
		...
		/**
		 * Angle of normal to line CCW from x axis with 0 and 180 being
		 * horizontal and 90 being vertical. thetaDeg spans -180 to 180.
		 */
	...
	private volatile LinearFeatureModel strongestCluster = null; // might be used by multiple threads
	...
	public void setMixingFactorTheta(float mixingFactor) {
		...
		mixingFactorTheta = mixingFactor;
	...
	/**
	 * If true, cluster lifetime withtout support increases proportional to the
	 * age of the cluster relative to the clusterLifetimeWithoutSupportUs time
	 */
	...
	/**
	 * Sets tne number of events to consider for forming line segments for
	 * clustering
	 *
	...
	public void setThetaRadiusDeg(float thetaRadius) {
		...
		thetaRadiusRad = (float) Math.toRadians(thetaRadius);
		getPrefs().putFloat("MultiLineClusterTracker.thetaRadiusRad", thetaRadiusRad);
	 ...
	 *
	 * @return angle in degrees. Ranges from -180 to 180 degrees, where -180 and
	 * 180 represent a vertical line (horizontal line normal) and 90 and -90 is
	...
	/**
	 * computes the output parameters using the given (assumed non null) line
	 * cluster. Transforms the ooordinates of the line to the chip image center
