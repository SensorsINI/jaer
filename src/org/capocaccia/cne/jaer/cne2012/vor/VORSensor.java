...
public class VORSensor extends EventFilter2D implements FrameAnnotater, Observer {
	...
	public int getSampleIntervalMs() throws PhidgetException {
		if (spatial.isAttached()) {
			sampleIntervalMs = spatial.getDataRate();
	...
	public void setSampleIntervalMs(int ms) {
		...
		try {
			...
			sampleIntervalMs = ms;
			getSupport().firePropertyChange("sampleIntervalMs", old, sampleIntervalMs);
			putInt("sampleIntervalMs", ms);
			log.info("set sample interval to " + ms + " ms");
		} catch (PhidgetException ex) {
			log.log(Level.WARNING, "can''t set interval {0}: {1}", new Object[]{sampleIntervalMs, ex.toString()});
		}
	}
