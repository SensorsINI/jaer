...
public class CUDAObjectTrackerControl extends EventFilter2D implements FrameAnnotater {
	...
	public void setInputPort(int inputPort) {
		...
		getSupport().firePropertyChange("inputPort", recvOnPort, inputPort);
		recvOnPort = inputPort;
	...
	public void setOutputPort(int outputPort) {
		...
		getSupport().firePropertyChange("outputPort", sendToPort, outputPort);
		sendToPort = outputPort;
	...
	public class GaborTemplate extends CUDATemplate implements PropertyChangeListener {
		...
		public void computeValues() {
			...
			for (int i = 0; i < size; i++) {
				y = -gaborXYMAX + (((i * gaborXYMAX) / (size - 1)) * 2);
				for (int j = 0; j < size; j++) {
					x = -gaborXYMAX + (((j * gaborXYMAX) / (size - 1)) * 2);
