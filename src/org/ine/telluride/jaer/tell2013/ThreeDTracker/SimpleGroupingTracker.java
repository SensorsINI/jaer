...
public class SimpleGroupingTracker extends EventFilter2D implements FrameAnnotater {
    ...
    public SimpleGroupingTracker(AEChip chip) {
        super(chip);
...
*/
    ...
    private void updateWeightAngle(int trackID, int k, float ts, float cosAngle) {
        if (blockConstraints[trackID][k] > 0) {
            ...
            if ((cosAngle > this.directionThresh) && (dt < kernelConst)) {
            ...
            if (e instanceof TrackerEvent) {
                ...
                if (randomMove == 1) {
                    ...
                    for (k=0; k<numGaussians; k++) {
                        w[trackID][k] = w[k][trackID] = this.startw;
...
/*                            System.out.println("K " + gaussTracker.getCenterX(k) + " / " + gaussTracker.getCenterY(k) +
            ...
            else System.out.println("No TrackerEvent");
        } // BasicEvent e
