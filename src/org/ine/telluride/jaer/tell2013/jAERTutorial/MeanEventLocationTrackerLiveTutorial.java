...
import org.capocaccia.cne.jaer.cne2011.*;
...
public class MeanEventLocationTrackerLiveTutorial extends EventFilter2D implements FrameAnnotater {
    ...
    private float mixingRateStd = getFloat("mixingRateStd", 0.01f); // how much we mix the new value into the running means
    private float radiusOfTransmission = getFloat("radiusOfTransmission", 10); // how big around mean location we transmit events

    public MeanEventLocationTrackerLiveTutorial(AEChip chip) {
        super(chip);
        setPropertyTooltip("mixingRate", "rate that mean location is updated in events. 1 means instantaneous and 0 freezes values");
        setPropertyTooltip("mixingRateStd", "rate that the standard deviation is updated in events. 1 means instantaneous and 0 freezes values");
    ...
    public EventPacket<?> filterPacket(EventPacket<?> in) {
        for (BasicEvent o : in) { // iterate over all events in input packet
            ...
            xstd = (1 - mixingRateStd) * xstd + Math.abs(o.x - xmean) * mixingRateStd;
            ystd = (1 - mixingRateStd) * ystd + Math.abs(o.y - ymean) * mixingRateStd;
        }
     ...
     * @return the mixingRate
     */
    public float getMixingRateStd() { // the getter and setter beans pattern allows introspection to build the filter GUI
        return mixingRateStd;
    }

    /**
     * @param mixingRate the mixingRate to set
     */
    public void setMixingRateStd(float mixingRateStd) {
        float old=this.mixingRateStd;
        this.mixingRateStd = mixingRateStd;
        putFloat("mixingRateStd", mixingRateStd); // stores the last chosen value in java preferences
        getSupport().firePropertyChange("mixingRateStd", old, mixingRateStd);
    }    
