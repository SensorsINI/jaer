 ...
 */
@Description("Tracks multiple Gaussians to fit clusters of events") // adds this string as description of class for jaer GUIs
public class GaussianTracker extends EventFilter2D implements FrameAnnotater {

    private int numGaussians = getInt("numGaussians", 20);
    ...
    public GaussianTracker(AEChip chip) {
        super(chip);
        ...
        for (int i=0; i<=N; i++) {
            circleX[i] = (float) (gaussRadius * Math.cos(i*2.0*Math.PI/N));
            circleY[i] = (float) (gaussRadius * Math.sin(i*2.0*Math.PI/N));
        }
    ...
    private void alloc_gaussians() {
        ...
        for (int i=0; i<numGaussians;i++) {
            float H = (float) i * 360.0f / ((float) numGaussians);
    ...
    public void annotate(GLAutoDrawable drawable) {
        if (drawCircles) {
            ...
            for (k=0; k<numGaussians; k++) {
                ...
                for (i=0; i<=N; i++) {
                    gl.glVertex2f((float) (centerX[k]+circleX[i]), (float) (centerY[k]+circleY[i]));
                }
                gl.glEnd();
            }
        }
