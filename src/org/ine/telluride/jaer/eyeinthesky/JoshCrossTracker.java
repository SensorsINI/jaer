...
public class JoshCrossTracker extends EventFilter2D implements FrameAnnotater {
	...
	private boolean updateCoefficients(PolarityEvent e) {
		...
		iWeight = 1.0 - weight;
		length = (iWeight * length) + (weight * ((2.0*distC)-ignoreRadius)); //calculate length (based on fact we ignore events too close to center)
	...
	private static void drawCircle( GL2 gl, double xc, double yc ) {
		double r = 2; // Radius.
