 ...
 * @author tobi
 */
public class GimbalCalibrator implements PropertyChangeListener, FrameAnnotater {
	static Logger log=Logger.getLogger("net.sf.jaer");
	CalibratedGimbal calibratedPanTilt;
	private boolean calibrating=false;
	transient GimbalGUI gui;
	float[][] retinaSamples=null;
	float[][] panTiltSamples=null;
	// sampled pan tilt values for corners
	boolean calibrated=false;
	Vector<GimbalCalibrationPoint> sampleVector=new Vector<GimbalCalibrationPoint>();
	float[][] transform=new float[][]{{1, 0, 0}, {0, 1, 0}};
	float[] computedPanTilt=new float[2];
                    private String who = "";

	public GimbalCalibrator(CalibratedGimbal panTilt) {
		super();
		calibratedPanTilt=panTilt;
                                         who = "GimbalCalibrator";
	...
	void paint(Graphics g) {
		final int r=6;
		for(GimbalCalibrationPoint p : sampleVector) {
			Point mp=gui.getMouseFromPanTilt(p.pt);
			g.drawOval(mp.x-r, mp.y-r, r*2, r*2);
		}
	}

	void addSample(GimbalCalibrationPoint sample) {
	...
	void calibrate() {
		calibrating=true;
		calibratedPanTilt.getPanTiltHardware().acquire(who);
		if(gui==null) {
			gui=new GimbalGUI(calibratedPanTilt.getPanTiltHardware(), this);
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if(evt.getPropertyName().equals(GimbalGUI.Message.AddSample.name())) {
			GimbalCalibrationPoint p=(GimbalCalibrationPoint) evt.getNewValue();
			addSample(p);
			computeCalibration();
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.ComputeCalibration.name())) {
			computeCalibration();
			log.info("computing calibration");
			saveCalibrationPrefs();
			calibratedPanTilt.getPanTiltHardware().release(who);
			setCalibrating(false);
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.ClearSamples.name())) {
			sampleVector.clear();
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.RevertCalibration.name())) {
			loadCalibrationPrefs();
			calibratedPanTilt.getPanTiltHardware().release(who);
			setCalibrating(false);
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.EraseLastSample.name())) {
			if(sampleVector.size()>0) {
				Object s=sampleVector.lastElement();
				sampleVector.remove(s);
			}
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.ShowCalibration.name())) {
			showCalibration();
		} else if(evt.getPropertyName().equals(GimbalGUI.Message.ResetCalibration.name())) {
	...
	private void loadCalibrationPrefs() {
		...
		try {
			...
			if(bytes!=null) {
				...
				sampleVector=(Vector<GimbalCalibrationPoint>) in.readObject();
				in.close();
