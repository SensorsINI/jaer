...
public class Gimbal implements GimbalInterface, LaserOnOffControl {
   ...
   private String lockOwner = "None";
    ...
    private float defaultRoll = 0 ;
     private float defaultPitch= 0 ;
      ...
      private float chipXFOV = 10; // degrees
       private float chipYFOV = 10;
     
    
    float MaxMovePerUpdate = 10f; //Max amount of gimbal per update
...
// Ensure gimbal controller is ready
        ...
        private static class SingletonHelper {
            private static final Gimbal INSTANCE = new Gimbal();
        }
        
        // Public method to provide access to the instance
        public static synchronized Gimbal getInstance() {
...
//    }
    ...
    /**
     *  The gimbal is an independent device whose absolute pose may be set by various agents.
     *  Therefore we add our required deltas to its current pose. 
     * 
     * @param pan
     * @param tilt 
     */
    private void setPose(float pan, float tilt) {
        float chipFOV = 50.0f ; // degrees temporary TODO get fromchip = rjd
        float deltaYaw =  (float) (pan - 0.5)*chipFOV ;  // yaw and pitch in degrees
        float deltaPitch =  (float) (tilt - 0.5)*chipFOV ;
         log.info("**********  deltaYaw, deltaPitch =  " + deltaYaw + "  deg / " + deltaPitch +" deg");
        // only send signifcant updates of pose
      //  needs a mod here!!  if (deltaYaw >= 0.01f || deltaPitch >=  0.01f ) 
     
        gimbal.setDeltaPosition(deltaYaw, 0.0f, deltaPitch);  // PanTilt does not considerRoll
       //log.info("Updated gimbal values to " + newYaw + "  deg / " + newPitch +" deg");
    ...
    public void setMoveUpdateFreqHz(int UpdateFreq) {
        if(UpdateFreq > 100) UpdateFreq = 100;  // ** rjd 
    ...
    public com.inilabs.jaer.gimbal.Gimbal.Trajectory getPanTiltTrajectory() {
    ...
    public com.inilabs.jaer.gimbal.Gimbal.Trajectory getPanTiltTargetTrajectory() {
