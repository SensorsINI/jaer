...
public class NewGimbalTracker extends EventFilter2DMouseAdaptor implements FrameAnnotater {
	...
	public NewGimbalTracker(AEChip chip) {
		super(chip);
		...
		if(tracker.getNumClusters()>0) {
			...
			if(c.isVisible()) {
				...
				//	panTilt.setPanTiltVisualAim(p.x, p.y);
                                                                                                    panTilt.setPanTiltVisualAimPixels(p.x, p.y);
				} catch(Exception ex) {
