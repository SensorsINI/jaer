...
public class GimbalAimerGUI extends javax.swing.JFrame implements PropertyChangeListener, ExceptionListener {
    ...
    class Trajectory extends ArrayList<TrajectoryPoint> {
        ...
        void add(float pan, float tilt, int x, int y) {
            ...
            add(new TrajectoryPoint(now - lastTime, pan, tilt, x, (h - y)));
            lastTime = now;
        }
        ...
        private void paintCrossHair(Color LineColor) {
            ...
            int n = size();
            int x = get(n-1).x, y = h - get(n-1).y;
            Graphics g = calibrationPanel.getGraphics();
            g.setColor(LineColor);
            g.drawLine(x-5,(h - y),x+5,(h - y));
            g.drawLine(x,(h - y)-5,x,(h - y)+5);
            
            g.drawLine(x-10,(h - y)+5,x-10,(h - y)+10);g.drawLine(x-10,(h - y)-5,x-10,(h - y)-10);
            g.drawLine(x+10,(h - y)+5,x+10,(h - y)+10);g.drawLine(x+10,(h - y)-5,x+10,(h - y)-10);
            
            g.drawLine(x+5,(h - y)-10,x+10,(h - y)-10);g.drawLine(x-5,(h - y)-10,x-10,(h - y)-10);
            g.drawLine(x+5,(h - y)+10,x+10,(h - y)+10);g.drawLine(x-5,(h - y)+10,x-10,(h - y)+10);
        }
    ...
    public GimbalAimerGUI(GimbalBase pt) {
        panTilt = pt;
    ...
    private void initComponents() {
        ...
        calibrationPanel.setPreferredSize(new java.awt.Dimension(1024, 768));
