...
public class FaceTrack extends EventFilter2D implements FrameAnnotater, Observer {
	...
	public void resetFilter() {
		...
		int x = (LED2[0] - LED1[0]) * (LED2[0] - LED1[0]);
		int y = (LED2[1] - LED1[1]) * (LED2[1] - LED1[1]);
		...
		for (int i = 0; i < (face.length + mouth_boundary.length); i++) {
			if (i < face.length)
			{
	...
	public EventPacket<?> filterPacket(EventPacket<?> in) {
		...
		for (Object e : in) {
			...
			x_data[numEventsCollected] = i.x;
			y_data[numEventsCollected] = i.y;
			numEventsCollected++;
