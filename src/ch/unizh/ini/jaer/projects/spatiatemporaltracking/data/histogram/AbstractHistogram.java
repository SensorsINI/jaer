...
public abstract class AbstractHistogram implements Histogram {
	...
	@Override
	public void init() {
		gaussian = new float[(2 * window) + 1];
		float s = 0;
		for (int i = -window; i <= window; i++) {
			double p = -Math.pow(i, 2) / 2;
			double e = Math.exp(p);
			gaussian[i + window] = (float)((1 / Math.sqrt(2*Math.PI)) * e);
			s += gaussian[i + window];
		}
	...
	public void draw(GLAutoDrawable drawable, TextRenderer renderer, float x, float y, int height, int resolution) {
		...
		if (drawAllBins) {
			...
			from = 0;
			float total = 0;
			while ((total < 0.1) && (from < getSize())) {
				total += getNormalized(from);
				from++;
			}
			to = from;
			from = Math.max(0, from - 2);
			while ((total < 0.9) && (to < getSize())) {
				total += getNormalized(to);
				to++;
			}
			to = Math.min(to + 2, nBins);
		}
		...
		String s=String.format("range [%d,%d], N=%d, entropy=%.2f",start + (from * step),start + (to * step),getN(),computeEntropy());
