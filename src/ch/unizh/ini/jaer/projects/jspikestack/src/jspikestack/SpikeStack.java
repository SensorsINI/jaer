 ...
 * @author oconnorp
 */
public class SpikeStack<LayerType extends SpikeStack.Layer> {
    
    // <editor-fold defaultstate="collapsed" desc=" Properties ">
    
    ArrayList<LayerType> layers=new ArrayList<LayerType>();
    
    Queue<Event> inputBuffer = new LinkedList<Event>();
    Queue<Event> internalBuffer= new LinkedList<Event>();
    
    double time;    // Current time (millis) (avoids having to pass around time reference)
        
    float tau;      // Decay rate (seconds)
        
    float tref=0;   // Abs-Refractory period
        
    float delay=0;  // 
    
    boolean liveMode=false;     // Live-mode.  If true, it prevents the network from advancing as long as the input buffer is empty
    
    boolean inputCurrents=false;  
    ...
    float inputCurrentStrength=1; 
    /* If inputCurrents==true, this is the strength with which input events drive
     * input layer units.
     */
        
    NetworkReader read=new NetReader();    // 
    NetPlotter plot;   // An object for displaying the state of the network
...
//            layers[i].units=new Layer.Unit[ini.layers[i].nUnits];
    ...
    public void feedEventsAndGo(List<Event> inputEvents)
    {   feedEvents(inputEvents);
        eatEvents();
    }
        
    /** Feed an array of input events to the network and let 'er rip */
    public void feedEventsAndGo(List<Event> inputEvents,double timeout)
    {   
        feedEvents(inputEvents);
        eatEvents(timeout);
    }
    ...
    public void feedEvents(List<Event> inputEvents)
    {   
...
//        inputBuffer=inputEvents;
        ...
        while (!(inputBuffer.isEmpty()&&(internalBuffer.isEmpty() || liveMode )))
        {
            ...
            Event ev=readInput?inputBuffer.poll():internalBuffer.poll();
    ...
    public int nLayers()
    {   // Just in case we want to change layers implementation.
        return layers.size();        
    }
    ...
    public static class Layer <NetType extends SpikeStack,LayerType extends SpikeStack.Layer, UnitType extends SpikeStack.Layer.Unit> 
    {
        NetType net;
        ...
        short dimx;     // For display, purposes.
        short dimy;     // dimx,dimy must multiply to units.length
        ...
        public ArrayList<Event> outBuffer=new ArrayList<Event>();
        ...
        public class Unit
        {
            ...
            {   
                ...
                outBuffer.add(new Event(ixUnit,net.time,ixLayer));
                     
                // Add to internal buffer at delay time.  (Wasteful repetition?)
                net.internalBuffer.add(new Event(ixUnit,net.time+net.delay,ixLayer));
...
//                if (Lin!=null) 
        ...
        public Event getOutputEvent(int outputBufferLocation)
