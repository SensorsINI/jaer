...
public class NetPlotter {
    ...
    int updateMillis=30;           // Update interval, in milliseconds
    float timeScale=1;               // Number of network-seconds per real-second 0: doesn't advance.  Inf advances as fast as CPU will allow
    
    boolean realTime=false;         // This will cause updates to be in real-time (scaled by timeScale)
    ...
    {
        // Add the data
        Iterator<Event> itr=lay.outBuffer.iterator();
...
//        System.out.println(net.time);
            ...
            {   Event ev=layer.getOutputEvent(outBookmark);
                if (ev.time>toTime)
                    break;
