...
public class AEPacket3D extends AEPacket {
  ...
  //  public int lastCaptureLength=0;
    ...
    private Event3D event=new Event3D();
    ...
    public AEPacket3D(int[] coordinates_x, int[] coordinates_y, int[] disparities, int[] methods, int[] lead_sides, float[] values, int[] timestamps) {
        if((coordinates_x==null) || (coordinates_y==null) || (disparities==null) || (lead_sides==null)  || (methods==null) || (values == null) || (timestamps==null)) {
			return;
		}
        ...
        if(coordinates_x.length!=timestamps.length) {
			throw new RuntimeException("coordinates_x.length="+coordinates_x.length+"!=timestamps.length="+timestamps.length);
		}
        if(coordinates_y.length!=timestamps.length) {
			throw new RuntimeException("coordinates_y.length="+coordinates_y.length+"!=timestamps.length="+timestamps.length);
		}
        if(disparities.length!=timestamps.length) {
			throw new RuntimeException("disparities.length="+disparities.length+"!=timestamps.length="+timestamps.length);
		}
        if(methods.length!=timestamps.length) {
			throw new RuntimeException("methods.length="+methods.length+"!=timestamps.length="+timestamps.length);
		}
        if(lead_sides.length!=timestamps.length) {
			throw new RuntimeException("lead_sides.length="+lead_sides.length+"!=timestamps.length="+timestamps.length);
		}
     ...
     public AEPacket3D(int[] coordinates_x, int[] coordinates_y, int[] coordinates_z,  float[] values, int[] timestamps) {
        if((coordinates_x==null) || (coordinates_y==null) || (coordinates_z==null) || (values == null) || (timestamps==null)) {
			return;
		}
        ...
        if(coordinates_z.length!=timestamps.length) {
			throw new RuntimeException("coordinates_z.length="+coordinates_z.length+"!=timestamps.length="+timestamps.length);
		}
    ...
    public synchronized int[] ensureCapacity(int[] array, final int c) {
       // super.ensureCapacity(c);
        if(array==null) {
            ...
            int newcap=2*c; // check ENLARGE_CAPACITY_FACTOR
