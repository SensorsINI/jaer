/*
 * StereoOnFundamentalMatrix4.java fast version using ordering constraint
 ...
 * @author rogister
 */
public class StereoOnFundamentalMatrix4 extends EventFilter2D implements FrameAnnotater, Observer /*, PreferenceChangeListener*/ {
...
//    protected final int NODISPARITY = -999;
 ...
 //   private int brightness=getPrefs().getInt("StereoOnFundamentalMatrix4.brightness",2);

//    private int minDiff=getPrefs().getInt("StereoOnFundamentalMatrix4.minDiff",14);
//    {setPropertyTooltip("minDiff","[microsec (us)] minimum delta between matching events");}


    private int realTimeBin=getPrefs().getInt("StereoOnFundamentalMatrix4.realTimeBin",20);
    {setPropertyTooltip("realTimeBin","time bin for real time, after it events are dropped");}



    private int decayTimeLimit=getPrefs().getInt("StereoOnFundamentalMatrix4.decayTimeLimit",10000);
    {setPropertyTooltip("decayTimeLimit","[microsec (us)] for decaying accumulated events");}

     private int deltaTime=getPrefs().getInt("StereoOnFundamentalMatrix4.deltaTime",1000);
    {setPropertyTooltip("deltaTime","[microsec (us)] max allowed difference between matching events");}

//    private float mix=getPrefs().getFloat("StereoOnFundamentalMatrix4.mix",0.5f);
//    {setPropertyTooltip("mix","mix==1 : only new disparity matters, mix==0, only previous, can be set between 0-1");}
//
//     private int disparityMin = getPrefs().getInt("StereoOnFundamentalMatrix4.disparityMin",20);
//    {setPropertyTooltip("disparityMin","disparity range constraint");}
     private int disparityMax = getPrefs().getInt("StereoOnFundamentalMatrix4.disparityMax",20);
    {setPropertyTooltip("disparityMax","disparity limit");}

       private int spreadingRange = getPrefs().getInt("StereoOnFundamentalMatrix4.spreadingRange",20);
    {setPropertyTooltip("spreadingRange","unset disparities are assigned neighbor disp in range");}


//
//     private int coherenceRadius = getPrefs().getInt("StereoOnFundamentalMatrix4.coherenceRadius",2);
//    {setPropertyTooltip("coherenceRadius","radiius to check coherence toward neighboring disparities");}
//    private float coherenceThreshold=getPrefs().getFloat("StereoOnFundamentalMatrix4.coherenceThreshold",3.0f);
//    {setPropertyTooltip("coherenceThreshold","threshold on neighboring disparity coherence");}

 
 private boolean checkOrdering = getPrefs().getBoolean("StereoOnFundamentalMatrix4.checkOrdering",false);



  //  private boolean decayEveryFrame = getPrefs().getBoolean("StereoOnFundamentalMatrix4.decayEveryFrame",false);
    //private boolean initGray = getPrefs().getBoolean("StereoOnFundamentalMatrix4.initGray",false);

  //  private float threshold=getPrefs().getFloat("StereoOnFundamentalMatrix4.threshold",0.4f);
  //  {setPropertyTooltip("threshold","threshold on acc values for generating new events");}

     private float maxDistance=getPrefs().getFloat("StereoOnFundamentalMatrix4.maxDistance",4f);
     private float maxDiff=getPrefs().getFloat("StereoOnFundamentalMatrix4.maxDiff",10f);



     //private boolean scaleAcc = getPrefs().getBoolean("StereoOnFundamentalMatrix4.scaleAcc",false);
 ...
 //  DPoint accRightPoints[][] = new DPoint[retinaSize][retinaSize];
   ...
   int disparities[][] = new int[retinaSize][retinaSize];
...
// do not use vectors...
 ...
 BinocularDisparityEvent leftEventList[];
  // counters to keep track of how many events we have in arrays this time, need be global
    ...
    public StereoOnFundamentalMatrix4(AEChip chip) {
        super(chip);
        this.chip=chip;
...
//        }
   ...
   //     Vector<BinocularEvent> leftEventList = new Vector<BinocularEvent>();
        ...
        // need decay isntead but for now just reset for every incoming packets
        disparities = new int[retinaSize][retinaSize];
        ...
        for(int index=0;index<nbProcessed;index++){
                BinocularDisparityEvent e = leftEventList[index];
                if(disparities[e.x][e.y]!=0){//alas for the moment no other check on validity
                    generateEvent(e,disparities[e.x][e.y]);
                } else {
                    // find closest event with disparity
                    int disp = closestDisparity(e.x,e.y);
                    if(disp<127){
                        generateEvent(e,disp);
                    }
                }
...
//        }
    ...
    int closestDisparity( int x, int y){
        //within range
        int range = spreadingRange;
        float minDist = 1000;
        int minD = 0;
                ...
                for (int j = y - range; j < y + range + 1; j++) {
                    if (j > 0 && j < retinaSize) {
                        if(disparities[i][j]!=0){
                            float dist = computeDist(x,y,i,j);
                            if(dist<minDist){
                                minD = disparities[i][j];
                                minDist = dist;
                            }
    ...
    float computeDist(int x1, int y1, int x2, int y2){
        float dist = (float)Math.sqrt(x1*x1 - x2*x2 + y1*y1 - y2*y2);
        return dist;
    }
    ...
    protected void processEvent(BinocularDisparityEvent e, BinocularDisparityEvent[] in){
           ...
           // to find points at minimum distance to epipolar line :
           // - compute epipolar line
           float[] epipolarLine = computeEpipolarLineTranspose(e.x, e.y);
           // - find closest point to epipolar line

           BinocularDisparityEvent me = findClosestEvent(in, e.x, e.y, sign, e.timestamp, deltaTime, epipolarLine);
           ...
           //
               ...
               // save disparity
               disparities[e.x][e.y] = me.x - e.x;
                // ordering constraint 2
               if(e.x-1>0){
                if(disparities[e.x-1][e.y]>disp&&disparities[e.x-1][e.y]!=0){
                    disparities[e.x][e.y] = disparities[e.x-1][e.y];
                    disp = disparities[e.x][e.y];
                }
               }
             ...
             //  generateEvent(e,disp);
               ...
               // ordering constraint 2
               if(e.x+1<retinaSize){
                    if(disparities[e.x+1][e.y]<disparities[e.x][e.y]){
                        disparities[e.x+1][e.y]=disparities[e.x][e.y];
                    }
               }
                if(e.x+2<retinaSize){
                    if(disparities[e.x+2][e.y]<disparities[e.x][e.y]){
                        disparities[e.x+2][e.y]=disparities[e.x][e.y];
                    }
               }
  ...
  protected BinocularDisparityEvent findClosestEvent( BinocularDisparityEvent[] in, int x, int y,  int sign, int time, int delta, float[] d){
     ...
     //     int eye = e.eye == BinocularEvent.Eye.LEFT ? 0 : 1;
          ...
          if (eye == RIGHT) {
               ...
               if (type == sign){ //||!usePolarity) { //with polarity check

                 if (Math.abs(time - e.timestamp) < delta) {

                   dist = (float)Math.abs(d[0] * e.x + d[1] * e.y + d[2]) / (float)Math.sqrt(d[0] * d[0] + d[1] * d[1]);
                   if (dist < mindist) {
                      if( nbEventsRight[e.x][e.y] !=0  && e.x > x && e.disparity!=-1){ // use d==-1 to tell if an event is already matched
                          if(Math.abs(e.x-x)<disparityMax){
...
//    void drawGLCluster(int x1, int y1, int x2, int y2)
   ...
   public void setRealTimeBin(int realTimeBin) {
        ...
        getPrefs().putInt("StereoOnFundamentalMatrix4.realTimeBin",realTimeBin);
    }
    ...
    public void setDeltaTime(int deltaTime) {
        ...
        getPrefs().putInt("StereoOnFundamentalMatrix4.deltaTime",deltaTime);
    }
...
//
//        getPrefs().putFloat("StereoOnFundamentalMatrix4.threshold",threshold);
//    }
...
//
//        getPrefs().putInt("StereoOnFundamentalMatrix4.brightness",brightness);
//    }

//     public void setDisparityMin(int disparityMin) {
//        this.disparityMin = disparityMin;
//
//        getPrefs().putInt("StereoOnFundamentalMatrix4.disparityMin",disparityMin);
//    }
...
//
    public void setDisparityMax(int disparityMax) {
        ...
        getPrefs().putInt("StereoOnFundamentalMatrix4.disparityMax",disparityMax);
    }
    public int getDisparityMax() {
        return disparityMax;
    }
    public void setSpreadingRange(int spreadingRange) {
        this.spreadingRange = spreadingRange;

        getPrefs().putInt("StereoOnFundamentalMatrix4.spreadingRange",spreadingRange);
    }
    public int getSpreadingRange() {
...
//
//        getPrefs().putInt("StereoOnFundamentalMatrix4.minDiff",minDiff);
//    }
...
//
//        getPrefs().putInt("StereoOnFundamentalMatrix4.coherenceRadius",coherenceRadius);
//    }
...
//
//        getPrefs().putFloat("StereoOnFundamentalMatrix4.coherenceThreshold",coherenceThreshold);
//    }
//
    ...
    public void setMaxDistance(float maxDistance) {
        ...
        getPrefs().putDouble("StereoOnFundamentalMatrix4.maxDistance",maxDistance);
    }
    ...
    public void setMaxDiff(float maxDiff) {
        ...
        getPrefs().putDouble("StereoOnFundamentalMatrix4.maxDiff",maxDiff);
    }
    ...
    public void setCheckOrdering(boolean checkOrdering){
        ...
        getPrefs().putBoolean("StereoOnFundamentalMatrix4.checkOrdering",checkOrdering);
    }
