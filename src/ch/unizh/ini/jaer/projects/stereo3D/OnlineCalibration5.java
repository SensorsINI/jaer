 ...
 * @author rogister
 */
public class OnlineCalibration5 extends EventFilter2D implements FrameAnnotater, Observer /*, PreferenceChangeListener*/ {
   ...
   private int timeWindow=getPrefs().getInt("OnlineCalibration5.timeWindow",1000);
  {setPropertyTooltip("timeWindow","timeWindow in us");}
   
   private int accThreshold=getPrefs().getInt("OnlineCalibration5.accThreshold",10);
  {setPropertyTooltip("accThreshold","minimum amount of events per pixel for matching");}


//  private int cleaningThreshold=getPrefs().getInt("OnlineCalibration5.cleaningThreshold",1);
//  {setPropertyTooltip("cleaningThreshold","minimum acc value below which cell is deleted to free memory");}

  private int maxThreshold=getPrefs().getInt("OnlineCalibration5.maxThreshold",1);
 // {setPropertyTooltip("maxThreshold","minimum max value f");}

  private int skipPixels=getPrefs().getInt("OnlineCalibration5.skipPixels",1);
  {setPropertyTooltip("skipPixels","optimize memory usage by skipping left pixels every n skipPixels");}



   private float intensityZoom = getPrefs().getFloat("OnlineCalibration5.intensityZoom",0.05f);
    {setPropertyTooltip("intensityZoom","zoom for association display window");}

   private float intensityZoom2 = getPrefs().getFloat("OnlineCalibration5.intensityZoom2",2);
    {setPropertyTooltip("intensityZoom2","zoom for 128*128 display window");}


    private int maxProcessTimeNs=getPrefs().getInt("OnlineCalibration5.maxProcessTimeNs",500000);
  {setPropertyTooltip("maxProcessTimeNs","max processing time allowed in nano sec");}

   private int factor=getPrefs().getInt("OnlineCalibration5.factor",10);

 //   private int currentTracker=getPrefs().getInt("OnlineCalibration5.currentTracker",0);
//  {setPropertyTooltip("currentTracker","index oc current tracker for each left points");}


  private int leftColumnX=getPrefs().getInt("OnlineCalibration5.leftColumnX",65);

        
 //   private int minEvents = getPrefs().getInt("OnlineCalibration5.minEvents",100);
//    {setPropertyTooltip("minEvents","min events to create GC");}
  
    private boolean showWindow1 = getPrefs().getBoolean("OnlineCalibration5.showWindow1", true);
    private boolean showWindow2 = getPrefs().getBoolean("OnlineCalibration5.showWindow2", true);
    private boolean showWindow3 = getPrefs().getBoolean("OnlineCalibration5.showWindow3", true);
    ...
    private int columnheight = getPrefs().getInt("OnlineCalibration5.columnheight",10);
    private int columnsize = getPrefs().getInt("OnlineCalibration5.columnsize",50);
...
//   Vector<TimePixel> rightPoints = new Vector<TimePixel>();
   ...
   int currentStartLeft = 0;
   int currentStartRight = 0;
   int currentLeft = 0;
   int currentRight = 0;
   int maxPoints = 10000;
   int[] leftPointsIndexes = new int[maxPoints];
   int[] rightPointsIndexes = new int[maxPoints];
   long[] leftPointsTimes = new long[maxPoints];
   long[] rightPointsTimes = new long[maxPoints];



    /** Creates a new instance of GravityCentersImageDumper */
    public OnlineCalibration5(AEChip chip) {
        super(chip);
        this.chip=chip;
      ...
      //  long startSysTime = System.currentTimeMillis();
        ...
        //    currentSysTime = System.currentTimeMillis();
            ...
            if(currentSysTime>startSysTime+maxProcessTimeNs){
               // System.out.println("OnlineCalibration5 track: throwing away events, nbEventsProcessed "+nbEventsProcessed+"/"+ae.getSize());
                break;
            }
...
/*
    ...
    */
    ...
    synchronized public void resetFilter() {
        if(!firstRun){
       // System.out.println ("OnlineCalibration5 resetFilter ");
        logLeftAccPNG = false;
...
//        }
    ...
    protected void resetArrays(){
      ...
      //  rightPoints = new Vector<TimePixel>();
            ...
            leftPointsIndexes = new int[maxPoints];
    rightPointsIndexes = new int[maxPoints];
    leftPointsTimes = new long[maxPoints];
    rightPointsTimes = new long[maxPoints];
...
//        
        ...
        track(in);
          ...
          if (showWindow1)  associationdisplayCanvas.repaint();
          if (showWindow2)  leftdisplayCanvas.repaint();
           if (showWindow3) rightdisplayCanvas.repaint();
    ...
    protected void addToAssociation(BinocularEvent e){
        ...
        if (leftOrRight == RIGHT) {//LEFT
         ...
         //  leftPoints.add(new TimePixel(e.x,e.y,e.timestamp));
           ...
           leftPointsIndexes[currentLeft]=e.y*retinaSize+e.x;
           leftPointsTimes[currentLeft]=e.timestamp;
           currentLeft++;
           if(currentLeft>=maxPoints){
               currentLeft=0;
           }
           if(currentLeft==currentStartLeft){
               currentStartLeft++;
               // we are filling the buffer from the other side  so we push the start
           }
       
        //   Vector<TimePixel> toTrim = new Vector<TimePixel>();
           int i = currentStartRight;
           ...
           //for(TimePixel tp:rightPoints){
               ...
               int tpIndex = rightPointsIndexes[i];
               long tpTime = rightPointsTimes[i];
               i++;
               
               // associate with last right events
               if(e.timestamp-tpTime<timeWindow){
                  //  associations[index][tp.index]++;

                   
                    int xr = tpIndex%retinaSize;

                   // ryad constraint on order
                   // if(e.x<xr){

                      int yr = Math.round(tpIndex/retinaSize);
...
//                           }
                   ...
                   //toTrim.add(tp);
                   currentStartRight++;
                   if(currentStartRight>maxPoints)currentStartRight=0;
                   if(currentStartRight==currentRight)allBufferRead=true;
               }
        ...
        //   rightPoints.add(new TimePixel(e.x,e.y,e.timestamp));
          ...
          // int index = e.y*retinaSize+e.x;
            ...
            rightPointsIndexes[currentRight]=e.y*retinaSize+e.x;
           rightPointsTimes[currentRight]=e.timestamp;
           currentRight++;
           if(currentRight>=maxPoints){
               currentRight=0;
           }
           if(currentRight==currentStartRight){
               currentStartRight++;
               // we are filling the buffer from the other side  so we push the start
           }

        //   Vector<TimePixel> toTrim = new Vector<TimePixel>();
           int i = currentStartLeft;
           boolean allBufferRead = false;
           ...
           while(!allBufferRead){
               ...
               int tpIndex = leftPointsIndexes[i];
               long tpTime = leftPointsTimes[i];
               i++;
         ...
         //  for(TimePixel tp:leftPoints){
               ...
               if(e.timestamp-tpTime<timeWindow){
                  //  associations[tp.index][index]++;
                     // ryad constraint on order
                    int xL = tpIndex%retinaSize;
                   // if(xL<e.x&&xL==leftColumnX){
                    if(xL==leftColumnX){

                      int yL = Math.round(tpIndex/retinaSize);
...
//                           }
                   ...
                   // trim
                     currentStartLeft++;
                   if(currentStartLeft>maxPoints)currentStartLeft=0;
                   if(currentStartLeft==currentLeft)allBufferRead=true;
    ...
    int closestAssociation( int index){
       ...
       // boolean found=false;
        while(nbtry<maxNbtry){
            ...
            if (decrease) newindex = newindex-1;
    ...
    void checkAssociationDisplayFrame(){
        if(showWindow1 && associationdisplayFrame==null) createAssociationDisplayFrame();
    }
...
//    GLUT glut=null;
    void createAssociationDisplayFrame(  ){
        ...
        associationdisplayCanvas.addGLEventListener(new GLEventListener(){
            ...
            private void drawAssociationsPoints(int[][] points, GL gl) {
          //  private void drawAllPoints(Hashtable associations, GL gl) {
    ...
    void checkLeftDisplayFrame(){
        if(showWindow2 && leftdisplayFrame==null) createLeftDisplayFrame();
    }
...
//System.out.println(" got index: "+highlightIndex);
        ...
        leftdisplayCanvas.addGLEventListener(new GLEventListener(){
          ...
          private void drawFittingLineL( int[][] points, int xL, int yL, float r, float g, float b, GL gl){
             ...
             // debug
                 ...
                 float linew = maxcumul/columnsize;
         ...
         // plot
             ...
             private void drawLeftPoints(int[][][] points, GL gl) {
    ...
    // accurate pixel frame
     ...
     void checkRightDisplayFrame(){
        if(showWindow3 && rightdisplayFrame==null) createRightDisplayFrame();
    }
...
//
//        getPrefs().putInt("OnlineCalibration5.skipPixels",skipPixels);
//    }
//
...
//
//        getPrefs().putInt("OnlineCalibration5.currentTracker",currentTracker);
//    }
//
...
//
    ...
    public void setMaxThreshold(int maxThreshold) {
        ...
        getPrefs().putInt("OnlineCalibration5.maxThreshold",maxThreshold);
    }
...
//
//        getPrefs().putInt("OnlineCalibration5.cleaningThreshold",cleaningThreshold);
//    }
//
...
//
//        getPrefs().putInt("OnlineCalibration5.accThreshold",accThreshold);
//    }
//
...
//    }
     ...
     public void setfactor(int factor) {
        ...
        getPrefs().putInt("OnlineCalibration5.factor",factor);
    }
   ...
   public void setLeftColumnX(int leftColumnX) {
        ...
        resetColumns();
        getPrefs().putInt("OnlineCalibration5.leftColumnX",leftColumnX);
    }
    ...
    public int getLeftColumnX() {
      ...
      public void setmaxProcessTimeNs(int maxProcessTimeNs) {
        ...
        getPrefs().putInt("OnlineCalibration5.maxProcessTimeNs",maxProcessTimeNs);
    }
    ...
    public int getmaxProcessTimeNs() {
     ...
     public void setColumnsize(int columnsize) {
        ...
        resetColumns();
        getPrefs().putInt("OnlineCalibration5.columnsize",columnsize);
    }
    ...
    public void setColumnheight(int columnheight) {
        ...
        resetColumns();
        getPrefs().putInt("OnlineCalibration5.columnheight",columnheight);
    }
    ...
    public void setTimeWindow(int timeWindow) {
        ...
        getPrefs().putInt("OnlineCalibration5.timeWindow",timeWindow);
    }
    ...
    public void setIntensityZoom(float intensityZoom) {
        ...
        getPrefs().putFloat("OnlineCalibration5.intensityZoom",intensityZoom);
    }
    ...
    public void setIntensityZoom2(float intensityZoom2) {
        ...
        getPrefs().putFloat("OnlineCalibration5.intensityZoom2",intensityZoom2);
    }
    ...
    public float getIntensityZoom2() {
     ...
     public void setShowWindow1(boolean showWindow1){
        ...
        getPrefs().putBoolean("OnlineCalibration5.showWindow1",showWindow1);
    }
    public boolean isShowWindow1(){
     ...
     public void setShowWindow2(boolean showWindow2){
        ...
        getPrefs().putBoolean("OnlineCalibration5.showWindow2",showWindow2);
    }
    public boolean isShowWindow2(){
     ...
     public void setShowWindow3(boolean showWindow3){
        ...
        getPrefs().putBoolean("OnlineCalibration5.showWindow3",showWindow3);
    }
