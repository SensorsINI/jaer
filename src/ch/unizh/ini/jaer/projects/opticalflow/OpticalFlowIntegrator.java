...
import ch.unizh.ini.jaer.projects.opticalflow.OpticalFlowIntegrator.Pixel;
...
public class OpticalFlowIntegrator
{
    ...
    public class GraphTrace {
        ...
        public void glDraw(GL gl) {
            ...
            int j;
            for(j=0; j<SIZE; j++)
                gl.glVertex3f((float) j,data[(i+j)%SIZE],0f);
            gl.glEnd();          
        }
    }
    ...
    public class Pixel {
        ...
        public float getValue() {
            float ret= 0f;
            for(int i=0; i<vs.length; i++) //HACK
...
//            System.err.println("x,y="+currentX()+","+currentY()+"; "+x1+","+y1+"-"+x2+","+y2);
   ...
   public synchronized void glTransform(GLAutoDrawable drawable,GL gl) {
    ...
    protected void glDrawTraces(GL gl) {
        if (!showTraces)
            return;
     ...
     * @param gain 
     */
    public synchronized void glDrawPixels(GL gl,float offset,float gain) {
        int dx= -currentX();
        int dy= -currentY();
    ...
    public void setDebugParam1(String value) {
        if (value.equals(""))
            ...
            this.debugParam1 = Float.parseFloat(value);
        prefs.putFloat("debugParam1", this.debugParam1);
    }
    ...
    public void setDebugParam2(String value) {
        if (value.equals(""))
            ...
            this.debugParam2 = Float.parseFloat(value);
        prefs.putFloat("debugParam2", this.debugParam2);
    }
