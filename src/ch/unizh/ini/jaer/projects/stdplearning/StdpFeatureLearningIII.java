...
public class StdpFeatureLearningIII extends RectangularClusterTracker implements Observer, FrameAnnotater, PropertyChangeListener {
    ...
    private void updateClusters() {
        int[] tempClusterID = new int[numClusters];             // Holds indices for visible clusters in clusters
        ...
        for (Cluster c : clusters) {
            if (c.isVisible()) {
                tempClusterID[idx] = c.getClusterNumber();
                if (idx<numClusters) {
                    idx++;
                } else {
                    break;
        ...
        for (int i=0; i<idx; i++) {
            for (int cID=0; cID<numClusters; cID++) {
                if (clusterID[cID] == tempClusterID[i]) {
                    tempClusterFound[i] = true;
                    clusterFound[cID] = true;
                    xStart[cID] = (int) (clusters.get(tempClusterID[i]).getLocation().x-xPixels/2f);
                    yStart[cID] = (int) (clusters.get(tempClusterID[i]).getLocation().y-yPixels/2f);
    ...
    void createNeuronFrame() {
        ...
        neuronFrame.setPreferredSize(new Dimension(400, 200));
                    ...
                    for (int n=0; n<neuronsPerCluster; n++) {
                        ...
                        for (int x=0; x<xPixels; x++) {
                            for (int y=0; y<yPixels; y++) {
                                ...
                                float wOFF = (synapseWeights[c][n][0][x][y] - wMin) / (float) (wMax - wMin);
                                float wON = (synapseWeights[c][n][1][x][y] - wMin) / (float) (wMax - wMin);
                                if (displayCombinedPolarity == true) {
                                    gl.glColor3f(wON, 0, wOFF);
                                    gl.glRectf(neuronOffset+x, clusterOffset+y+yPixels, 
                                            neuronOffset+x+1, clusterOffset+yPixels+1);
