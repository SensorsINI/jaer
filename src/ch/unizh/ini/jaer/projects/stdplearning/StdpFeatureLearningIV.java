...
public class StdpFeatureLearningIV extends RectangularClusterTracker implements Observer, FrameAnnotater, PropertyChangeListener {
    ...
    // Neurons
    private int neurons;                    // Number of Neurons in each cluster
    ...
    void createNeuronFrame() {
        ...
        neuronCanvas.addGLEventListener(new GLEventListener() {
            ...
            synchronized public void display(GLAutoDrawable drawable) {
                ...
                int neuronsPerRow = (int) Math.sqrt((double) neurons); 
                ...
                for (int n=0; n<neurons; n++) {
                    for (int x=0; x<xPixels; x++) {
                        for (int y=0; y<yPixels; y++) {
                            ...
                            float wOFF = (synapseWeights[n][0][x][y] - wMin) / (float) (wMax - wMin);
                            float wON = (synapseWeights[n][1][x][y] - wMin) / (float) (wMax - wMin);
