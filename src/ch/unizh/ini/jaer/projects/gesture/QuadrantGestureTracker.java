...
public class QuadrantGestureTracker extends EventFilter2D implements FrameAnnotater, Observer /*, PreferenceChangeListener*/ {
	...
	public void initFilter() {
		...
		defaultClusterRadius=Math.max(chipSizeX,chipSizeY)*getClusterSize();  //added PW
	}
	...
	void checkHardware(){    //added PW
		if(hw==null) {
			hw=new WowWeeAliveHardwareInterface();
		}
		try{
			if(!hw.isOpen()) {
				hw.open();
	...
	public class Cluster{
		...
		private final void scale(BasicEvent event){
			if(!dynamicSizeEnabled && !dynamicAspectRatioEnabled) {
				return;
			}
			if(dynamicSizeEnabled){
