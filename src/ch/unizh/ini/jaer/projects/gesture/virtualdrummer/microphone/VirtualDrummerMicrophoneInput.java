...
/**
 * Uses the microphone input to detect and generate
 * {@link DrumBeatSoundEvent}'s. This object is a thread that reads the
 * microphone input and generates beats from it. To use this class, make a new
 * instance, register {@link DrumBeatSoundEventListener}'s, and
 * {@link #startReporting} the thread. To stop reporting, use
 * {@link #stopReporting}.
 *
 * <p>
 * An instance of this class is constructed when microphone recording is
 * activated; listeners are added there.
 * <p>
 * A beat is positive edge triggered by the microphone input going above the
 * {@link #setThreshold}; a new beat is not generated until the input falls
 * below the threshold-{@link #setHystersis hystersis level}.
 *
 *
 *
 * @author $Author: tobi $
...
//    private float readbufferRatio = .5f;
    ...
    /**
     * Creates a new instance of test. Opens the microphone input as the target
     * line. To start the reporting, {@link #start} the thread.
     *
...
//        getAudioInfo();  // prints lots of useless information
    ...
    /**
     * starts acquisition from microphone port and generation of
     * {@link DrumBeatSoundEvent}'s. If line is not available it does nothing.
     */
    ...
    /**
     * removes all beat event listeners, ends thread after first stopping
     * microphone acquisition. This ends generation of
     * {@link DrumBeatSoundEvent}'s
     */
    ...
    /**
     * grabs samples from microphone input and generates
     * {@link DrumBeatSoundEvent}'s whenver spikes are detected. Stopped by
     * {@link #stopReporting}
     */
    @Override
    public void run() {
...
//                        System.out.println("mic recorder: spike");
    ...
    /**
     * add a listener for all spikes. Listeners are
     * {@link DrumBeatSoundEventListener#drumBeatSoundOccurred(ch.unizh.ini.jaer.projects.gesture.virtualdrummer.microphone.DrumBeatSoundEvent)  called}
     * when a spike occurs and are passed a {@link DrumBeatSoundEvent}.
     *
    ...
    /**
     * sets the hystersis for beat detection. Set this to avoid triggering
     * multiple beats on a noisy signal. A new {@link DrumBeatSoundEvent} can be
     * not generated until the input drops below the
     * {@link #getThreshold threshold}-hystersis.
     */
