 ...
 *
 */
public class EllipseTracker extends EventFilter2D implements FrameAnnotater {
...
//craete eyeMaskFrame
        if (eyeMaskTop > (eyeMaskXright - eyeMaskXleft) / 2) {
            eyeMaskTop = Math.round((eyeMaskXright - eyeMaskXleft) / 2 - 1);
        }
        if (eyeMaskBottom > (eyeMaskXright - eyeMaskXleft) / 2) {
            eyeMaskBottom = Math.round((eyeMaskXright - eyeMaskXleft) / 2 - 1);
        }
...
//craete eyeMaskArray: if inside eyeMask -> eyeMask(i,j)=1 else eyeMask(i,j)=0
        ...
        for (int x = 0; x < chip.getSizeX(); x++) {
            for (int y = 0; y < chip.getSizeY(); y++) {
                ...
                if (y >= eyeMaskY - eyeMaskBottom
                    ...
                    float sinPhi = 0;
                    if (deltaR != 0) {
                        cosPhi = deltaX / deltaR;
                        sinPhi = deltaY / deltaR;
                    }
    ...
    public void setirisRadius(float irisRadius) {
        if (irisRadius < 0) {
            irisRadius = 0;
        } else if (irisRadius > chip.getMaxSize()) {
            irisRadius = chip.getMaxSize();
        }
    ...
    public void setpupilRadius(float pupilRadius) {
        if (pupilRadius < 0) {
            pupilRadius = 0;
        } else if (pupilRadius > chip.getMaxSize()) {
            pupilRadius = chip.getMaxSize();
        }
    ...
    public void setcameraToEyeDistanceMM(float cameraToEyeDistanceMM) {
        if (cameraToEyeDistanceMM < 0) {
            cameraToEyeDistanceMM = 0;
        }
    ...
    public void seteyeCenterX(int eyeCenterX) {
        if (eyeCenterX < 0) {
            eyeCenterX = 0;
        } else if (eyeCenterX > chip.getMaxSize()) {
            eyeCenterX = chip.getMaxSize();
        }
    ...
    public void seteyeCenterY(int eyeCenterY) {
        if (eyeCenterY < 0) {
            eyeCenterY = 0;
        } else if (eyeCenterY > chip.getMaxSize()) {
            eyeCenterY = chip.getMaxSize();
        }
    ...
    public void seteyeMaskXleft(int eyeMaskXleft) {
        if (eyeMaskXleft < 0) {
            eyeMaskXleft = 0;
        } else if (eyeMaskXleft > eyeMaskXright) {
            eyeMaskXleft = eyeMaskXright - 1;
        }
    ...
    public void seteyeMaskXright(int eyeMaskXright) {
        if (eyeMaskXright > chip.getSizeX()) {
            eyeMaskXright = chip.getSizeX();
        } else if (eyeMaskXright < eyeMaskXleft) {
            eyeMaskXright = eyeMaskXleft + 1;
        }
    ...
    public void seteyeMaskY(int eyeMaskY) {
        if (eyeMaskY < 0) {
            eyeMaskY = 0;
        } else if (eyeMaskY > chip.getSizeX()) {
            eyeMaskY = chip.getSizeX();
        }
    ...
    public void seteyeMaskTop(int eyeMaskTop) {
        if (eyeMaskTop < 0) {
            eyeMaskTop = 1;
        } else if (eyeMaskTop + eyeMaskY > chip.getSizeX()) {
            eyeMaskTop = chip.getSizeX() - eyeMaskY;
        }
    ...
    public void seteyeMaskBottom(int eyeMaskBottom) {
        if (eyeMaskBottom < 0) {
            eyeMaskBottom = 0;
        } else if (eyeMaskY - eyeMaskBottom < 0) {
            eyeMaskBottom = eyeMaskY;
        }
            ...
            if (eyeMaskArray[x][y]) {
                ...
                if (accumulatorArray[x][y] >= maxConv) {
                    ...
                    if (maxConv > treshold && (x != maxCoordinate.x || y != maxCoordinate.y)) {
                        if (x - filteredMaxX > maxStepSize) {
                            filteredMaxX = filteredMaxX + maxStepSize;
                        } else if (x - filteredMaxX > -maxStepSize) {
                            filteredMaxX = x;
                        } else {
                            filteredMaxX = filteredMaxX - maxStepSize;
                        }

                        if (y - filteredMaxY > maxStepSize) {
                            filteredMaxY = filteredMaxY + maxStepSize;
                        } else if (y - filteredMaxY > -maxStepSize) {
                            filteredMaxY = y;
                        } else {
                            filteredMaxY = filteredMaxY - maxStepSize;
                        }
                        ...
                        float sinPhi = 0;
                        if (rdelta != 0) {
                            cosPhi = deltaX / rdelta;
                            sinPhi = deltaY / rdelta;
                        }
...
// d(x-1,y+1) = -2b^2x+b^2-2cy-2c+2a^2y+a^2+2cx = d(x,y+1)+slopeY+BB
    ...
    synchronized public EventPacket filterPacket(EventPacket in) {
        ...
        for (Object o : in) {
            ...
            if (!eyeMaskArray[event.x][event.y]) {
                continue;
            }
            ...
            if (event.x < 0 || event.y < 0) {
                continue; // wait until the ringbuffer is filled
            }
    ...
    synchronized public void setLogDataEnabled(boolean logDataEnabled) {
        ...
        if (dataLogger == null) {
            dataLogger = new EventFilterDataLogger(this, "targetX targetY eyeX eyeY");
        }
