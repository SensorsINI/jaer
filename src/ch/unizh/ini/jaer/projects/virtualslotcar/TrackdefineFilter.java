...
public class TrackdefineFilter extends EventFilter2D implements FrameAnnotater, Observer, MouseListener, MouseMotionListener, PropertyChangeListener {
	...
	public void update(Observable o, Object arg) {
		if ((o instanceof AEChip) && ((arg == Chip2D.EVENT_SIZEX) || (arg == Chip2D.EVENT_SIZEY))) {
			resetFilter();
	...
	synchronized public void doExtractTrack() {
		...
		extractPoints.add(new Point2D.Float(maxX, maxY));
		...
		for (int i = 0; i < numY; i++) {
			for (int j = 0; j < numX; j++) {
				if (trackPoints[i][j]) {
					// Insert into queue
					float dist = (float) Point2D.distance(maxX, maxY, j, i);
					if (dist > minDistance) {
