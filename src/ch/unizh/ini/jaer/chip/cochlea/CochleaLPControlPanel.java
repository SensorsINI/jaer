...
public final class CochleaLPControlPanel extends javax.swing.JPanel implements Observer {
	 ...
	 *
	 */
	private static final long serialVersionUID = 5664233857321567551L;
	Preferences prefs = Preferences.userNodeForPackage(CochleaLPControlPanel.class);
	Logger log = Logger.getLogger("CochleaAMS1cControlPanel");
	CochleaLP chip;
	private final CochleaAMS1c.Biasgen biasgen;
	...
	public CochleaLPControlPanel(final CochleaLP chip) {
		...
		final Integer max = new Integer(getBiasgen().getScanner().nstages - 1);
		...
		for (final CochleaAMS1c.Biasgen.AbstractConfigValue bit : biasgen.config) {
			if (bit instanceof CochleaAMS1c.ConfigTristate) {
				final ConfigTristate b2 = (ConfigTristate) bit;
				final JPanel panel = new JPanel();
				panel.setLayout(new BoxLayout(panel, BoxLayout.X_AXIS));
				panel.setAlignmentX(Component.LEFT_ALIGNMENT);
				final JLabel label = new JLabel(bit.getName() + ": " + bit.getDescription());
				final JRadioButton but = new JRadioButton("Set/Clear");
				but.setToolTipText("<html>" + b2.toString() + "<br>Select to set bit, clear to clear bit");
				final JRadioButton hiZButton = new JRadioButton("HiZ");
				...
				final ConfigBit b2 = (ConfigBit) bit;
				final JRadioButton but = new JRadioButton(bit.getName() + ": " + bit.getDescription());
				...
				final ConfigInt in = (ConfigInt) bit;
				final JPanel pan = new JPanel();
				pan.setAlignmentX(Component.LEFT_ALIGNMENT);
				pan.setLayout(new BoxLayout(pan, BoxLayout.X_AXIS));
				final JLabel label = new JLabel(in.getName());
				label.setToolTipText("<html>" + in.toString() + "<br>" + in.getDescription()
		...
		for (final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel c : biasgen.equalizer.channels) {
			final EqualizerControls cont = new EqualizerControls(new QSOSSlider(c), new QBPFSlider(c),
				new LPFKillBox(c), new BPFKillBox(c));
	...
	private void setSelectedChannel(final int channel) {
	...
	public void setScanX(final int scanX) {
		scanner.setScanX(scanX);
	}

	public void setScanContinuouslyEnabled(final boolean scanContinuouslyEnabled) {
	...
	class TristateableConfigBitButtons {
		...
		TristateableConfigBitButtons(final JRadioButton zb, final JRadioButton hb) {
			zeroOneButton = zb;
			hiZButton = hb;
		}
	}
	...
	private class EqualizerControls {
		...
		EqualizerControls(final QSOSSlider qSOSSlider, final QBPFSlider qBPFSlider, final LPFKillBox lPFKillBox,
			final BPFKillBox bPFKillBox) {
	...
	final Dimension sliderDimPref = new Dimension(2, 200), sliderDimMin = new Dimension(1, 35),
		killDimPref = new Dimension(2, 25), killDimMax = new Dimension(6, 15), killDimMin = new Dimension(1, 15);
	 ...
	 */
	@Override
	public void update(final Observable observable, final Object object) { // thread safe to ensure gui cannot retrigger
																			// this while
		// it is sending something
		// log.info(observable + " sent " + object);
		try {
			if (observable instanceof ConfigTristate) {
				final ConfigTristate b = (ConfigTristate) observable;
				final TristateableConfigBitButtons but = tristateableButtonsMap.get(b);
				if (b.isHiZ()) {
				...
				final ConfigBit b = (ConfigBit) observable;
				final AbstractButton but = (AbstractButton) configBitMap.get(b);
				but.setSelected(b.isSet());
				// log.info("set button for "+b+" to selected="+but.isSelected());
			}
			else if (observable instanceof ConfigInt) {
				final ConfigInt b = (ConfigInt) observable;
				final JTextField tf = (JTextField) configBitMap.get(b);
				tf.setText(Integer.toString(b.get()));
				// log.info("set button for "+b+" to selected="+but.isSelected());
			}
			else if (observable instanceof CochleaAMS1c.Biasgen.BufferIPot) {
				final BufferIPot bufferIPot = (BufferIPot) observable;
				...
				final EqualizerControls cont = eqMap.get(c);
				...
				// sends 0 byte message (no data phase for speed)
				final Equalizer c = (Equalizer) observable;
	...
	class QSOSSlider extends EqualizerSlider {
		 ...
		 *
		 */
		private static final long serialVersionUID = -699464730340213277L;

		QSOSSlider(final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel) {
			super(channel);
	...
	class QBPFSlider extends EqualizerSlider {
		 ...
		 *
		 */
		private static final long serialVersionUID = 975319792788261421L;

		QBPFSlider(final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel) {
			super(channel);
	...
	class BPFKillBox extends KillBox {
		 ...
		 *
		 */
		private static final long serialVersionUID = -47460688796583088L;

		BPFKillBox(final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel) {
			super(channel);
	...
	class LPFKillBox extends KillBox {
		 ...
		 *
		 */
		private static final long serialVersionUID = 5652613744041110745L;

		LPFKillBox(final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel) {
			super(channel);
	...
	class KillBox extends JButton {
		 ...
		 *
		 */
		private static final long serialVersionUID = -4802533869914820783L;
		...
		KillBox(final CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel) {
			...
			setOpaque(true);
			final MouseListener[] a = getMouseListeners();
			for (final MouseListener m : a) {
	...
	class EqualizerSlider extends JSlider {
		 ...
		 *
		 */
		private static final long serialVersionUID = 1815201897704748407L;
		CochleaAMS1c.Biasgen.Equalizer.EqualizerChannel channel;
			...
			addMouseListener(new MouseListener() {
				...
				public void mouseEntered(final MouseEvent e) {
					...
					if ((e.getModifiersEx() & InputEvent.BUTTON3_DOWN_MASK) == InputEvent.BUTTON3_DOWN_MASK) {
						final int v = (int) ((getMaximum() * (float) (getHeight() - e.getY())) / getHeight());
						setValue(v);
						setFileModified();
					}
				}
	...
	class ConfigTristateAction implements ActionListener {
		...
		ConfigTristateAction(final CochleaAMS1c.ConfigTristate bit) {
			this.bit = bit;
		}
	...
	class ConfigBitAction implements ActionListener {
		...
		ConfigBitAction(final CochleaAMS1c.ConfigBit bit) {
			this.bit = bit;
		}
	...
	class ConfigIntAction implements ActionListener {
		...
		ConfigIntAction(final CochleaAMS1c.ConfigInt bit) {
			integerConfig = bit;
		}
		...
		public void actionPerformed(final ActionEvent e) {
			...
			try {
				...
				final int val = Integer.parseInt(tf.getText());
	...
	private void initComponents() {
		...
		jLabel4
		.setText("<html>Sends a 48 bit input to the 2 daisy-chained DACs. <p>Enter the 12 character hex value and hit enter.</html>");
		dacCmdPanel.add(jLabel4);

		dacCmdComboBox.setEditable(true);
		dacCmdComboBox.setFont(new java.awt.Font("Courier New", 0, 11)); // NOI18N
		dacCmdComboBox
		.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "000000 000000", "ffffff ffffff" }));
		...
		org.jdesktop.beansbinding.Binding binding = org.jdesktop.beansbinding.Bindings.createAutoBinding(
			...
			org.jdesktop.beansbinding.ELProperty.create("${scanContinuouslyEnabled}"),
		...
		jPanel5
		.setToolTipText("Selects which of the two on-chip scanner sync outputs should be monitored to count scan location. Selection must be correct to monitor given type when scanning is stopped!");
		...
		jPanel5Layout.setHorizontalGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
			.addGroup(
				jPanel5Layout.createSequentialGroup().addContainerGap().addComponent(basmemBut)
				.addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED).addComponent(gangcellBut)
		...
		jPanel5Layout.setVerticalGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
			.addGroup(
				...
				.addGroup(
					...
					.addComponent(basmemBut).addComponent(gangcellBut))));

		final javax.swing.GroupLayout continuousScanningPanelLayout = new javax.swing.GroupLayout(
			continuousScanningPanel);
		continuousScanningPanel.setLayout(continuousScanningPanelLayout);
		continuousScanningPanelLayout.setHorizontalGroup(continuousScanningPanelLayout.createParallelGroup(
			javax.swing.GroupLayout.Alignment.LEADING).addGroup(
				continuousScanningPanelLayout
				.createSequentialGroup()
				.addGroup(
					continuousScanningPanelLayout
		...
		continuousScanningPanelLayout.setVerticalGroup(continuousScanningPanelLayout.createParallelGroup(
			javax.swing.GroupLayout.Alignment.LEADING).addGroup(
				continuousScanningPanelLayout
				.createSequentialGroup()
				.addComponent(continuousScanningEnabledCheckBox)
		...
		singleChannelSelectionPanel.setLayout(new javax.swing.BoxLayout(singleChannelSelectionPanel,
		...
		binding = org.jdesktop.beansbinding.Bindings.createAutoBinding(
			org.jdesktop.beansbinding.AutoBinding.UpdateStrategy.READ_WRITE, scanSpinner,
			org.jdesktop.beansbinding.ELProperty.create("${value}"), scanSlider,
		...
		binding = org.jdesktop.beansbinding.Bindings.createAutoBinding(
			org.jdesktop.beansbinding.AutoBinding.UpdateStrategy.READ_WRITE, this,
			org.jdesktop.beansbinding.ELProperty.create("${scanX}"), scanSpinner,
			org.jdesktop.beansbinding.BeanProperty.create("value"));
		bindingGroup.addBinding(binding);
		...
		jLabel6
		.setText("<html>Warnings: ADC must be enabled to run Scanner. <br> State changes made here can leave other GUI controls in inconsistent state");
		...
		final javax.swing.GroupLayout onchipPanelLayout = new javax.swing.GroupLayout(onchipPanel);
		onchipPanel.setLayout(onchipPanelLayout);
		onchipPanelLayout.setHorizontalGroup(onchipPanelLayout.createParallelGroup(
			javax.swing.GroupLayout.Alignment.LEADING).addGroup(
				onchipPanelLayout
				.createSequentialGroup()
				.addContainerGap()
				.addComponent(onchipGainPanel, javax.swing.GroupLayout.PREFERRED_SIZE,
					javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
					.addContainerGap(736, Short.MAX_VALUE)));
		onchipPanelLayout.setVerticalGroup(onchipPanelLayout.createParallelGroup(
			javax.swing.GroupLayout.Alignment.LEADING).addComponent(onchipGainPanel,
		...
		final javax.swing.GroupLayout preampPanelLayout = new javax.swing.GroupLayout(preampPanel);
		preampPanel.setLayout(preampPanelLayout);
		preampPanelLayout.setHorizontalGroup(preampPanelLayout.createParallelGroup(
			...
			.addGroup(
				...
				.addGroup(
					...
					.addComponent(offchipPanel, javax.swing.GroupLayout.Alignment.LEADING,
						javax.swing.GroupLayout.DEFAULT_SIZE, 978, Short.MAX_VALUE)
						.addComponent(onchipPanel, javax.swing.GroupLayout.Alignment.LEADING,
							javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
							Short.MAX_VALUE)).addContainerGap()));
		preampPanelLayout.setVerticalGroup(preampPanelLayout.createParallelGroup(
			javax.swing.GroupLayout.Alignment.LEADING).addGroup(
				preampPanelLayout
				.createSequentialGroup()
				.addComponent(onchipPanel, javax.swing.GroupLayout.PREFERRED_SIZE,
					javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
					.addGap(18, 18, 18)
					.addComponent(offchipPanel, javax.swing.GroupLayout.PREFERRED_SIZE, 241,
						javax.swing.GroupLayout.PREFERRED_SIZE).addContainerGap(215, Short.MAX_VALUE)));
	...
	private void tabbedPaneMouseClicked(final java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tabbedPaneMouseClicked
		prefs.putInt("CochleaAMS1cControlPanel.selectedPaneIndex", tabbedPane.getSelectedIndex());
	}// GEN-LAST:event_tabbedPaneMouseClicked

	private void bufferBiasSliderStateChanged(final javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_bufferBiasSliderStateChanged
	...
	private void dacCmdComboBoxActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_dacCmdComboBoxActionPerformed
		...
		catch (final HardwareInterfaceException he) {
	...
	private void dacCmdComboBoxItemStateChanged(final java.awt.event.ItemEvent evt) {// GEN-FIRST:event_dacCmdComboBoxItemStateChanged
	}// GEN-LAST:event_dacCmdComboBoxItemStateChanged

	private void initDACButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_initDACButtonActionPerformed
		try {
			getBiasgen().initDAC();
		}
		catch (final Exception e) {
	...
	private void dacPoweroffButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_dacPoweroffButtonActionPerformed
		try {
			getBiasgen().setDACPowered(!dacPoweroffButton.isSelected()); // ! because this should set powered off if
			// selected
		}
	...
	private void dacPoweronButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_dacPoweronButtonActionPerformed
		try {
	...
	private void offchipLeftGainLowButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipLeftGainLowButActionPerformed
	...
	private void offchipLeftGainMedButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipLeftGainMedButActionPerformed
	...
	private void offchipLeftGainHighButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipLeftGainHighButActionPerformed
	...
	private void offchipRightGainLowButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipRightGainLowButActionPerformed
	...
	private void offchipRightGainMedButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipRightGainMedButActionPerformed
	...
	private void offchipRightGainHighButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_offchipRightGainHighButActionPerformed
	...
	private void arFastButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_arFastButActionPerformed
		biasgen.setArRatio(CochleaAMS1c.OffChipPreamp_AGC_AR_Ratio.Fast);
	}// GEN-LAST:event_arFastButActionPerformed

	private void arMedButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_arMedButActionPerformed
		biasgen.setArRatio(CochleaAMS1c.OffChipPreamp_AGC_AR_Ratio.Medium);
	}// GEN-LAST:event_arMedButActionPerformed

	private void arSlowButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_arSlowButActionPerformed
		biasgen.setArRatio(CochleaAMS1c.OffChipPreamp_AGC_AR_Ratio.Slow);
	}// GEN-LAST:event_arSlowButActionPerformed

	private void onchipLowBActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_onchipLowBActionPerformed
		biasgen.getOnchipPreamp().setGain(CochleaAMS1c.OnChipPreampGain.Low);
	}// GEN-LAST:event_onchipLowBActionPerformed

	private void onchipMedBActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_onchipMedBActionPerformed
		biasgen.getOnchipPreamp().setGain(CochleaAMS1c.OnChipPreampGain.Medium);
	}// GEN-LAST:event_onchipMedBActionPerformed

	private void onchipHighBActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_onchipHighBActionPerformed
		biasgen.getOnchipPreamp().setGain(CochleaAMS1c.OnChipPreampGain.Higher);
	}// GEN-LAST:event_onchipHighBActionPerformed

	private void scanSpinnerStateChanged(final javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_scanSpinnerStateChanged
		continuousScanningEnabledCheckBox.setSelected(false);
	}// GEN-LAST:event_scanSpinnerStateChanged

	private void basmemButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_basmemButActionPerformed
		biasgen.getScanner().setScanBasMemV(basmemBut.isSelected());
	}// GEN-LAST:event_basmemButActionPerformed

	private void gangcellButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_gangcellButActionPerformed
		biasgen.getScanner().setScanGanglionCellVMem(gangcellBut.isSelected());
	}// GEN-LAST:event_gangcellButActionPerformed

	private void resetEqButActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_resetEqButActionPerformed
		biasgen.resetEqualizer();
	}// GEN-LAST:event_resetEqButActionPerformed

	private void onchipHighestBActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_onchipHighestBActionPerformed
		biasgen.getOnchipPreamp().setGain(CochleaAMS1c.OnChipPreampGain.Highest);
	}// GEN-LAST:event_onchipHighestBActionPerformed

	private void unkillallbpfbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_unkillallbpfbutActionPerformed
		biasgen.equalizer.setAllBPFKilled(false);
	}// GEN-LAST:event_unkillallbpfbutActionPerformed

	private void killallbpfbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_killallbpfbutActionPerformed
		biasgen.equalizer.setAllBPFKilled(true);
	}// GEN-LAST:event_killallbpfbutActionPerformed

	private void killalllpfbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_killalllpfbutActionPerformed
		biasgen.equalizer.setAllLPFKilled(true);
	}// GEN-LAST:event_killalllpfbutActionPerformed

	private void unkillalllpfbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_unkillalllpfbutActionPerformed
		biasgen.equalizer.setAllLPFKilled(false);
	}// GEN-LAST:event_unkillalllpfbutActionPerformed

	private void allmaxsosqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allmaxsosqualbutActionPerformed
		biasgen.equalizer.setAllQSOS(Equalizer.MAX_VALUE);
	}// GEN-LAST:event_allmaxsosqualbutActionPerformed

	private void allmidsosqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allmidsosqualbutActionPerformed
		biasgen.equalizer.setAllQSOS(Equalizer.MAX_VALUE / 2);
	}// GEN-LAST:event_allmidsosqualbutActionPerformed

	private void allminsosqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allminsosqualbutActionPerformed
		biasgen.equalizer.setAllQSOS(0);
	}// GEN-LAST:event_allminsosqualbutActionPerformed

	private void allmaxbpfqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allmaxbpfqualbutActionPerformed
		biasgen.equalizer.setAllQBPF(Equalizer.MAX_VALUE);
	}// GEN-LAST:event_allmaxbpfqualbutActionPerformed

	private void allmidbpfqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allmidbpfqualbutActionPerformed
		biasgen.equalizer.setAllQBPF(Equalizer.MAX_VALUE / 2);
	}// GEN-LAST:event_allmidbpfqualbutActionPerformed

	private void allminbpfqualbutActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_allminbpfqualbutActionPerformed
