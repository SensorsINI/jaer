...
/**
 * Displays data from CochleaAMS1c using RollingCochleaGramDisplayMethod with
 * additional rolling strip chart of ADC samples. There are really two distinct
 * display methods. One is the "strip chart" display of a large number of
 * samples vs. time, for example from the microphones or a particular BM
 * section. The other is the display of an "image" of the data from scanning out
 * the scanner data from the chip, showing all the BM sections at once.
 * <p>
 * Depending on the data in CochleaAMS1cADCSamples the data is handled
 * differently.
 * <p>
 * In the strip chart case, the data is acquired from the ADCSamples object,
 * resetting it in the process, and passed to the Series plotting object. In the
 * meantime the USB thread is filling the other ADCSamples buffer.
 * <P>
 * In the image case, the data in the ADCSamples object is not reset by the
 * acquisition. Rather, whatever data is in the ADCSamples object is passed to
 * Series after the Series has been reset. Thus the data shown represents the
 * latest data available, which could be a mixture of different "frames".
 *
 * @author Tobi
 */
