 ...
 * @author  tobi
 */
public class DACchannelTextControl extends JPanel implements Observer, StateEditable {
    ...
    static Preferences prefs = Preferences.userNodeForPackage(DACchannelTextControl.class);
     ...
     * 
     */
    public DACchannelTextControl(DACchannel dacchannel) {
        ...
        if (dacchannel != null) {
            ...
            slider.setMaximum(dacchannel.getMaxBitValue());
            slider.setMinimum(0);
            slider.setToolTipText(dacchannel.getTooltipString());
    ...
    public String toString() {
        return "DACchannelControl for pot " + dacchannel.getName();
    }
    ...
    protected void updateAppearance() {
        ...
        valueTextField.setText(engFormat.format(dacchannel.getVoltage()));
    }
    ...
    private int sliderValueFromBitValue(JSlider s) {
        ...
        int v = (int) Math.round(f * dacchannel.getMaxBitValue());
        return v;
    }
    // gets from the bit value the slider value

    private int bitValueFromSliderValue(JSlider s) {
        int v = (int) Math.round((float) dacchannel.getBitValue() / dacchannel.getMaxBitValue() * s.getMaximum());
        return v;
    }

    /** called when Observable changes (pot changes) */
    public void update(Observable observable, Object obj) {
        if (observable instanceof DACchannel) {
...
//        setBorder(selectedBorder);
    ...
    private void valueTextFieldMouseWheelMoved(java.awt.event.MouseWheelEvent evt) {//GEN-FIRST:event_valueTextFieldMouseWheelMoved
        ...
        startEdit(); 
        dacchannel.changeByFractionOfFullScale(ratio);
...
//        String s = evt.getKeyText(evt.getKeyCode());
        ...
        if (code == KeyEvent.VK_UP) {
            startEdit();
            dacchannel.changeByFractionOfFullScale(byRatio);
            endEdit();
...
//        System.out.println("slider state changed for "+pot);
        ...
        if (v == 0) {
            dacchannel.setBitValue(0); // these pot chanages will come back to us as Observer events
...
//        System.out.println("restore state");
        ...
        if (hashtable.get(STATE_KEY) == null) {
            log.warning("pot " + dacchannel + " not in hashtable " + hashtable + " with size=" + hashtable.size());
...
//        System.out.println(" storeState "+pot);
    ...
    public static boolean isVoltageEnabled() {
        return DACchannelTextControl.valueEnabled;
    }

    public static void setVoltageEnabled(final boolean voltageEnabled) {
        DACchannelTextControl.valueEnabled = voltageEnabled;
        prefs.putBoolean("DACchannelControl.voltageEnabled", voltageEnabled);
    }

    public static boolean isSliderEnabled() {
        return DACchannelTextControl.sliderEnabled;
    }

    public static void setSliderEnabled(final boolean sliderEnabled) {
        DACchannelTextControl.sliderEnabled = sliderEnabled;
        prefs.putBoolean("DACchannelControl.sliderEnabled", sliderEnabled);
    }
    static ArrayList<DACchannelTextControl> allInstances = new ArrayList<DACchannelTextControl>();

    public static void revalidateAllInstances() {
        for (DACchannelTextControl c : allInstances) {
//            System.out.println(c);
    ...
    static boolean sliderEnabled = prefs.getBoolean("DACchannelControl.sliderEnabled", true);
    static boolean valueEnabled = prefs.getBoolean("DACchannelControl.voltageEnabled", true);
}
