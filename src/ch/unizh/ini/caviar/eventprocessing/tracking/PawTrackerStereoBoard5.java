...
public class PawTrackerStereoBoard5 extends EventFilter2D implements FrameAnnotater, Observer /*, AE3DPlayerInterface, AE3DRecorderInterface*/ /*, PreferenceChangeListener*/ {
  ...
  //  private final int MAX_NB_FINGERS=5;
   ...
   int nbEventTracker = 0;
    ...
    private int maxNbEventTracker=getPrefs().getInt("PawTrackerStereoBoard5.maxNbEventTracker",1);
    ...
    // end class FingerCluster
     ...
     private class EventTracker{
          ...
          public void add( int x, int y, float mix, int time){
            ...
            if(dt>1){
              this.vx = Math.round(vx*velocity_mix + (x-this.x)/dt*vm1);
              this.vy = Math.round(vy*velocity_mix + (y-this.y)/dt*vm1);
            }
            this.time = time;
            this.x = Math.round(x*mix + this.x*m1);
            this.y = Math.round(y*mix + this.y*m1);
        ...
        private void doMatch(EventTracker et, EventTracker mt){
             // match
                        ...
                        Point3D p = triangulatePoint(et.side, et.x, et.y, mt.x, mt.y);
  ...
  //        System.out.println("right_focal after world "+right_focal.x+" "+right_focal.y+" "+right_focal.z);
   ...
   //     }
      ...
      synchronized private void clearDeadTrackers(int time){
          ...
          for (int i=0;i<maxTrackerLines;i++){
              clearDeadEventTrackers(time,leftOnTrackers[i]);
              clearDeadEventTrackers(time,leftOffTrackers[i]);
              clearDeadEventTrackers(time,rightOnTrackers[i]);
              clearDeadEventTrackers(time,rightOffTrackers[i]);
          }
   ...
   synchronized private void clearDeadPawTrackers(int time){
    ...
    synchronized private void clearDeadEventTrackers(int time, Vector<EventTracker> tracker){
    ...
    //    }
        ...
        if(closeTrackers.size()>0){
           ...
           // }
            ...
            if(nbEventTracker<maxNbEventTracker){
                nbEventTracker++;
...
//                }
    ...
    public synchronized void displayAllEventTrackers( GL gl ){
     ...
     public synchronized void displayEventTrackers(GL gl,  Vector<EventTracker> trackers, Color color ){
        
        int nbLines = retinaSize/trackerLineWidth; 
        float[] rgb=color.getRGBColorComponents(null);
        gl.glColor3f(rgb[0], rgb[1], rgb[2]);
        for(EventTracker et:trackers){
              drawEventTracker(gl, et, color);
        }   
     
    }
    
       final void drawEventTracker(GL gl, final EventTracker c, Color color){
        int x=c.x;
        int y=c.y;
