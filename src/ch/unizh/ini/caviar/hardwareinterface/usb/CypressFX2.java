...
public class CypressFX2 implements UsbIoErrorCodes, PnPNotifyInterface, AEMonitorInterface, ReaderBufferControl {
    ...
    // note .bix file format is deprecated. The new binary file format is .iic (i2c) format which are image files for the EEPROM
    
    /** filename of FX2 binary executable, generated by uVision project and associated script that runs hex2bix */
    public final static String FIRMWARE_FILENAME_TMPDIFF128_SMALL_CPLD_CLCC_BOARD="/ch/unizh/ini/caviar/hardwareinterface/usb/firmwareFX2_RetinaCPLD_JTAG.bix"; //"USBAEMonFirware.bin";
    /** filename of FX2 binary executable, generated by uVision project and associated script that runs hex2bix */
    ...
    class AsyncStatusThread extends Thread {
        ...
        public void run(){
            ...
            while(!stop && !isInterrupted()){
                ...
                if (status == 0) {
                    if(stop) log.info("Error submitting read on status pipe: "+UsbIo.errorText(buffer.Status));
                    break;
                }
                status=pipe.waitForCompletion(buffer);
                if (status != 0 && buffer.Status!=UsbIoErrorCodes.USBIO_ERR_CANCELED) {
                    if(!stop  && !isInterrupted()) log.warning("Error waiting for completion of read on status pipe: "+UsbIo.errorText(buffer.Status));
                    break;
                }
...
//                    + " Product ID (PID) " + HexString.toString((short)deviceDescriptor.idProduct));
        ...
        boolean success=false; int triesLeft=10;
        long delay=400;
...
//        System.out.println("CypressFX2RetinaBiasgen.unconfigureDevice()");
    ...
    protected String getFirmwareFilenameBinaryFromVIDPID() {
        ...
        if(getPID()==PID_TMPDIFF128_FX2_SMALL_BOARD){
            firmwareFilename=FIRMWARE_FILENAME_TMPDIFF128_SMALL_CPLD_CLCC_BOARD;
        } else if (getPID()==PID_USBAERmini2_without_firmware){
