...
public class PolarizationUtils {
    ...
    /**
     * Compute polarization (AoP, DoP) args: intensity: light intensity aop,
     * dop: buffers where the Aop/Dop will be updated (must be height x width //
     * 4) map: function to map the intensity: if the intensity comes from a log
     * sensor, then this function must be the exp, otherwise it's the identity
     * indexfX: index of the neighbour pixels of (i, j) inside his macro pixel
     */
    public static void computeAoPDoP(float[] intensity, float[] f0, float[] f45, float[] f90, float[] f135, float[] aop, float[] dop, FloatFunction map,
        ...
        if (indexf0.length != size || indexf45.length != size || indexf90.length != size || indexf135.length != size) {
            throw new IllegalArgumentException("Index array length should be height * width");
        }
        // Compute AoP and DoP
        for (int x = 0; x < width; x += 2) {
            for (int y = 0; y < height; y += 2) {
                int idx = getIndex(x, y, width);
                float ff0 = map.run(intensity[indexf0[idx]]);
                float ff45 = map.run(intensity[indexf45[idx]]);
                float ff90 = map.run(intensity[indexf90[idx]]);
                float ff135 = map.run(intensity[indexf135[idx]]);
                float s0 = (ff0 + ff135 + ff90 + ff45) / 2;
                float s1 = ff0 - ff90;
                float s2 = ff45 - ff135;
                idx = x / 2 + y / 2 * width / 2;  // change idx so it now points to correct place in f0/f45/.../dop/aop arrays
                f0[idx] = ff0;
                f45[idx] = ff45;
                f90[idx] = ff90;
                f135[idx] = ff135;
    ...
    /**
     * Display the polarization, and display the results in a 3n * m image where
     * [:n, :] corresponds to the DoP, [n:2n, :] corresponds to the legends and
     * [2n:, :] corresponds to the AoP
     */
