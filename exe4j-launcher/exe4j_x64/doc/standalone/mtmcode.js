...
function MTMTrackExpand(thisMenu) {
  ...
  for(i = 0; i < thisMenu.items.length; i++) {
    if(thisMenu.items[i].url != "" && MTMTrackTarget(thisMenu.items[i].target)) {
      ...
      comparedPath = thisMenu.items[i].url;
      var regExp2 = /\.\.\//g;
      comparedPath = comparedPath.replace(regExp2, "");
      if(targetPath.lastIndexOf(comparedPath) != -1 && (targetPath.lastIndexOf(comparedPath) + comparedPath.length) == targetPath.length) {
