...
4. **Prefer Ant over the IDE compiler for packaging a runnable tree.** Language-server classes go to `.jdt-output` (gitignored). Ant still owns `build/classes`. If a launch fails with `Unresolved compilation problem` inside `build/classes`, run `ant clean` then `ant compile`. Do not add an Eclipse `.project` that has only the Ivy nature; the Java language server will then ignore `src` (empty Outline, Ctrl-click does nothing) until you run **Java: Clean Java Language Server Workspace** and reload.
