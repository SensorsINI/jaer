...
* [OpenCV cameras](#opencv-cameras) — synchronized webcam / USB frames with an event sensor
...
Open a **UVC webcam or other OpenCV USB camera** in a second viewer and **File → Synchronize**, then record: RGB frames and the event sensor share one AEDAT-4 session (software sync, about one frame period). **Interface** lists those cameras next to libusb event cameras (`OpenCV: 0 DSHOW 640x480`, AVFoundation on macOS, V4L2 on Linux). Selecting one switches **Sensor** to `OpenCvFrameCamera`. Live RGB frames record as AEDAT-4 **FRME**. The **OpenCV** menu requests size, YUY2/MJPG, frame rate, brightness/contrast (mouse wheel or Left/Right), autofocus, and on Windows the DirectShow **Camera settings…** dialog.

Playback assigns FRME-only cameras (or OpenCV EVTS+FRME with the same `source`) to `OpenCvFrameCamera`. Frame/IMU-only files play on a time playhead (CountDuration); mixed files follow the event-camera slice. **Remember last interface** off plus Interface → **None** leaves the webcam free for other apps.

<img src="https://raw.githubusercontent.com/SensorsINI/jaer/master/release-notes/3.5.0/first-event-frame-multicamera.png" alt="OpenCV webcam beside a live event camera" width="80%" />

<!-- webp: 3.5.0/event-frame-multicam.webp -->
