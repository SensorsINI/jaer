<!--
  ...
  Screenshots still to capture into release-notes/3.5.0/:
    signed-windows.png (Properties â†’ Digital Signatures / no SmartScreen)
    ...
    sample-data-writable-folder.png (Help â†’ Sample data chooser when Program Files is not writable)
...
| macOS | Apple Silicon (M1â€“M4) | [jAER_macos_aarch64_3_5_0.dmg](https://github.com/SensorsINI/jaer/releases/download/3.5.0/jAER_macos_aarch64_3_5_0.dmg) |
...
Each installer is <250 MB and includes a bundled [Eclipse Temurin](https://adoptium.net/) JDK from Adoptium (same **25** LTS as 3.4.0) â€” you do not install Java yourself. GitHub lists the same files again under **Assets** at the bottom of this page.

Video: [installing and updating jAER on YouTube](https://youtu.be/qQVt8_gwYVY) (also covers *git clone* and rebuild from master).

jAER can self-update (Help â†’ Check for release updatesâ€¦ â†’ **Download and install**). Older archival releases may remain on [Dropbox](https://www.dropbox.com/scl/fo/ibqmrztay51g7fg5d7mu3/h?rlkey=ulwos9lxmv38rrv5x1flic9z2&dl=0).

Installers offer a **sample recordings** download (off by default, ~995 MB). You can also fetch them later from Help â†’ **Sample data**. Skipping or cancelling that download does not roll back the install.
...
Download the `.exe` and run it. **3.5.0 is Authenticode-signed** (publisher **Tobias Delbruck**). SmartScreen will still say *Windows protected your PC* until that signature has enough downloads â€” **More info** â†’ **Run anyway**. If **Smart App Control** blocks the launcher, allow the app or turn that feature off. `winget` can install this signed build even while SmartScreen still warns.
...
Apple menu â†’ About This Mac: **Chip** Apple M1â€“M4 â†’ `aarch64` DMG; **Processor** Intel â†’ `jAER_macos_3_5_0.dmg` (no `aarch64` in the name). Terminal: `uname -m` is `arm64` or `x86_64`.

**3.5.0 DMGs are signed with Apple Developer ID and notarized by Apple.** Gatekeeper treats them as from identified developer **Tobias Delbruck**. Double-click the `.dmg` (it mounts a disk). In the Finder window, double-click **`jAER 3.5.0 Installer`**. A notarized GitHub DMG should open normally. If you still have an older unsigned DMG, use [right-click â†’ Open](https://support.apple.com/guide/mac-help/open-a-mac-app-from-an-unidentified-developer-mh40616/mac) (or Privacy & Security â†’ **Open Anyway**). Prefer a user folder (`~/Applications` or `~/jaer`) unless you want an admin install into `/Applications`.
...
* [Signed Windows and macOS installers](#signed-installers) â€” Authenticode + notarized Developer ID
* [Timed / rotating VCR recordings](#vcr) â€” cassette files, merge on stop
* [Sample recordings](#sample-data) â€” installer + Help â†’ Sample data; writable folder when Program Files is locked
* [OpenCV cameras](#opencv-cameras) â€” synchronized webcam / USB frames with an event sensor (NRV + webcam)
* [Double-click AEDAT in one jAER](#double-click-aedat) â€” single instance; Windows / Linux / macOS file association
* [Export video](#export-video) â€” File name / folder, IN/OUT, ffmpeg MP4
* [Playback marks and time](#playback) â€” IN/OUT/m on the sparkline; analog clock; FlexTime min/max
* [Save As](#save-as) â€” background export; record-order AEDAT-4; taskbar ETA
* [Bug fixes](#bug-fixes-and-minor-improvements)

<h4 id="signed-installers">Signed Windows and macOS installers</h4>

Windows `.exe` media is **Authenticode-signed**. The publisher is **Tobias Delbruck** (the same name Gatekeeper shows on Mac). Keep using **More info** â†’ **Run anyway**; SmartScreen lags new signatures. `winget` can install this signed build even while that warning still appears.
...
**File â†’ Start recording** (and any timed start) opens a setup dialog: name, folder, AEDAT-4 compression, optional **recorded-event filtering**, and a **Time limit / VCR** section. A non-zero limit is the length of each cassette. VCR is AEDAT-4 only.

* **Infinite** â€” keep adding cassette files (`â€¦_c0001.aedat4`, `_c0002`, â€¦) in a `â€¦-VCR` session folder
* **Finite** â€” stop after N cassettes
* **Rotate** â€” keep only the last N files (oldest deleted as the next cassette opens)
...
<h4 id="export-video">File â†’ Export video</h4>

**File â†’ Export videoâ€¦** splits **File name** vs **Folder** (recent folders, same extension handling as Save As). Playback defaults to the open recordingâ€™s basename and parent folder; live capture uses `Chip_yyyy-MM-dd-HH-mm` in the last recording folder. Capture **rewinds to IN** (or start) and stops at **OUT** / EOF. Overwrite asks once (Donâ€™t show again). Optional ffmpeg convert to MP4 after close; odd canvas sizes no longer break the convert. Opening an exported `.mp4` / `.avi` from Recent Files launches the OS player instead of treating it as an event recording.
...
**File â†’ Preferences**: relative vs absolute playback time, **Always display time**, and an **analog clock** (absolute) or stopwatch (relative) at the lower-left of the chip view. Recording timezone is preserved for AEDAT-4 playback. HUD overlay fonts scale with chip size and stay in the viewport when zoomed.
...
* **File â†’ Synchronize** muxed recording flushes leftover USB/raw packets on timestamp zero (DAVIS, DVS128, EVK4, DVX, SciDVS, Cochlea, Prophesee, NRV, OpenCV) so the first packet is a real t=0 slice. Playback shares CountDuration, target FPS, and playhead; **R** / auto-repeat rewinds the whole group at the shortest stream.
...
* **Rendering-rate warning** no longer double-dialogs; Donâ€™t show again sticks.
...
* Python / File â†’ Remote guide: [docs/README-DNN-OpenCV-ROS.md](https://github.com/SensorsINI/jaer/blob/master/docs/README-DNN-OpenCV-ROS.md).
