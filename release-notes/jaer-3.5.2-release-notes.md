...
**jAER 3.5.2** is a point release after **[3.5.0](https://github.com/SensorsINI/jaer/releases/tag/3.5.0)** (there is no public 3.5.1). Click the **statistics bar** for a field legend, noise-filter overlays **fill the chip width**, **USB IN** numbers live in USB tuning, and **EVK4 / Prophesee** live-drop handling is first-class. Hardware Configuration opens on the **user-friendly** tab. Production installers use a **smaller JRE** and train an **ahead-of-time (AOT) cache** after the first launch. See [Highlights](#highlights) below.
...
The public landing page is **[jaerproject.org](https://jaerproject.org/)** (also [sensorsini.github.io/jaer](https://sensorsini.github.io/jaer/)). It detects your OS and offers **Download jAER for Windows / macOS / Linux**.
...
* [USB IN statistics](#usb-in) — live FIFO / fill / throughput in USB tuning
...
* [Smaller, faster installers](#installers) — jlinked JRE; ahead-of-time (AOT) cache after first launch
...
<h4 id="installers">Smaller JRE, ahead-of-time (AOT) cache</h4>

Production media **jlink** a JRE module set and strip unused install4j runtime classes (`shrinkRuntime`). The first Java 25 start after install trains a HotSpot **ahead-of-time (AOT)** file under `tmpdir/jaer/` (`%TEMP%\jaer\` on Windows). Later launches reuse that cache. On a GEEKOM laptop with Intel Core Ultra 9 185H (2.50 GHz), full startup from click to live camera dropped from **9 s to about 4 s**. Linux and macOS media no longer pack a leftover install4j setup download (~176 MB).
