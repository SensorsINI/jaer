...
| **[DSEC HDF5](https://dsec.ifi.uzh.ch/data-format/)** | `.h5` (`events.h5`) | cooked `/events/{p,t,x,y}`, `/ms_to_idx`, `/t_offset` | [DSEC](https://dsec.ifi.uzh.ch/) (UZH) | yes | no | no | Blosc + ZSTD (via jHDF + `BloscHdf5Filter`) | VGA 640×480 polarity — chip `DVS640` (left/right are separate files; open one at a time) |
