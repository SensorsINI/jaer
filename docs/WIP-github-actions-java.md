...
Status: **partial** — SignPath Windows workflow uses current action majors. `javac` target is **25** (same as the bundled JRE).
...
3. **install4j on CI** — Linux `build-linux.yml` is **13.1** (Mini). `build-macos-notarize.yml` / `build-win-sign.yml` still **13.0.2** until Linux size is confirmed.
